...
function tableStoreSalesSummary(summary) {
  ...
  const kpis = `<div class="grid four dash-kpis" style="margin-bottom:14px"><div class="card kpi"><div class="label">Doanh thu thực đạt</div><div class="num">${money(t.revenue || 0)}đ</div><div class="hint">Target tháng: ${money(summary.monthly_target || 0)}đ • ${percentBadge(t.achievement_percent || 0)}</div></div><div class="card kpi"><div class="label">Target ngày đã set</div><div class="num">${money(t.daily_target || 0)}đ</div><div class="hint">Theo tổng target ngày trong tháng • ${percentBadge(t.daily_achievement_percent || 0)}</div></div><div class="card kpi"><div class="label">UPT</div><div class="num">${fmt2(t.upt || 0)}</div><div class="hint">${targetBadge(t.upt || 0, t.target_upt || 0, '', 2)}</div></div><div class="card kpi"><div class="label">ATV</div><div class="num">${money(t.atv || 0)}đ</div><div class="hint">${targetBadge(t.atv || 0, t.target_atv || 0, 'đ')}</div></div><div class="card kpi"><div class="label">CR</div><div class="num">${Number(t.cr || 0)}%</div><div class="hint">${targetBadge(t.cr || 0, t.target_cr || 0, '%', 2)}</div></div><div class="card kpi"><div class="label">Lượt khách cửa hàng</div><div class="num">${money(t.customer_count || 0)}</div><div class="hint">CR = bill / lượt khách</div></div></div>`;
  const rows = summary.rows.map(r => `<tr><td><b>${dOnly(r.sale_date)}</b></td><td>${money(r.revenue || 0)}</td><td>${money(r.daily_target || 0)}</td><td>${percentBadge(r.daily_achievement_percent || 0)}</td><td>${money(r.bill_count || 0)}</td><td>${money(r.item_count || 0)}</td><td>${money(r.customer_count || 0)}</td><td>${targetBadge(r.upt || 0, r.daily_target_upt || r.target_upt || 0, '', 2)}</td><td>${targetBadge(r.atv || 0, r.daily_target_atv || r.target_atv || 0, 'đ')}</td><td>${targetBadge(r.cr || 0, r.daily_target_cr || r.target_cr || 0, '%', 2)}</td><td>${percentBadge(r.achievement_percent || 0)}</td><td>${esc(r.note || r.daily_target_note || '')}</td></tr>`).join('');
  return `${kpis}<div class="table-wrap"><table><thead><tr><th>Ngày</th><th>Doanh thu ngày</th><th>Target ngày</th><th>% ngày</th><th>Bill</th><th>Món</th><th>Lượt khách CH</th><th>UPT</th><th>ATV</th><th>CR</th><th>% lũy kế tháng</th><th>Ghi chú</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}


function tableWeeklyDays(rows) {
  if (!rows?.length) return '<div class="empty">Chưa có dữ liệu ngày trong tuần này</div>';
  return `<div class="table-wrap"><table><thead><tr><th>Ngày</th><th>Doanh thu</th><th>Target ngày</th><th>% đạt</th><th>Bill</th><th>Món</th><th>Lượt khách</th><th>UPT</th><th>ATV</th><th>CR</th><th>Ghi chú</th></tr></thead><tbody>${rows.map(r => `<tr><td><b>${dOnly(r.sale_date)}</b></td><td>${money(r.revenue || 0)}đ</td><td>${money(r.target_revenue || 0)}đ</td><td>${percentBadge(r.achievement_percent || 0)}</td><td>${money(r.bill_count || 0)}</td><td>${money(r.item_count || 0)}</td><td>${money(r.customer_count || 0)}</td><td>${fmt2(r.upt || 0)}</td><td>${money(r.atv || 0)}đ</td><td>${fmt2(r.cr || 0)}%</td><td>${esc(r.note || '')}</td></tr>`).join('')}</tbody></table></div>`;
}

function tableWeeklyEmployees(rows) {
  if (!rows?.length) return '<div class="empty">Chưa có dữ liệu cá nhân trong tuần này</div>';
  return `<div class="table-wrap"><table><thead><tr><th>Nhân viên</th><th>Doanh thu</th><th>Target tuần ước tính</th><th>% đạt</th><th>% tỷ trọng DT</th><th>Bill</th><th>Món</th><th>UPT</th><th>ATV</th></tr></thead><tbody>${rows.map(r => `<tr><td><b>${esc(r.full_name)}</b></td><td>${money(r.revenue || 0)}đ</td><td>${money(r.target || 0)}đ</td><td>${percentBadge(r.achievement_percent || 0)}</td><td>${Number(r.revenue_percent || 0)}%</td><td>${money(r.bill_count || 0)}</td><td>${money(r.item_count || 0)}</td><td>${fmt2(r.upt || 0)}</td><td>${money(r.atv || 0)}đ</td></tr>`).join('')}</tbody></table></div>`;
}
...
async function renderWeeklyReport() {
  ...
  shell(`<div class="card"><div class="toolbar"><div class="field"><label>Chọn ngày trong tuần</label><input class="input" id="weeklyDateFilter" type="date" value="${esc(data.week_start)}"></div>${storeFilter}<div class="field"><label>Tuần đang xem</label><div class="input readonly">${dOnly(data.week_start)} - ${dOnly(data.week_end)}</div></div><div style="align-self:end">${can('can_export') ? '<button class="btn secondary" id="weeklyExportBtn">Tải CSV báo cáo tuần</button>' : ''}</div></div><p class="hint">Chọn bất kỳ ngày trong tuần, hệ thống tự gom từ T2 đến CN. Lưu feedback xong sẽ hiện dạng bảng, bấm Sửa feedback mới mở ô nhập.</p></div>${kpis}<section class="grid two" style="margin-top:16px"><div class="card"><h3>Doanh thu theo ngày</h3>${tableWeeklyDays(data.days || [])}</div><div class="card"><h3>Doanh thu cá nhân</h3>${tableWeeklyEmployees(data.employees || [])}</div></section>${feedbackSection}`, 'Báo cáo tuần', `Tổng hợp tuần ${dOnly(data.week_start)} - ${dOnly(data.week_end)}`);
  ...
  $('#weeklyFeedbackForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await api('/api/weekly-report', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(e.target))) }); toast('Đã lưu feedback tuần'); state.weeklyEditMode = false; renderWeeklyReport(); } catch (err) { toast(err.message, 'danger'); } });
}
...
async function renderSales() {
  ...
  const targetForm = can('can_set_sales_targets') ? `<div class="card"><h3>Set target đầu tháng từng nhân viên</h3><p class="hint">Chỉ tài khoản được cấp quyền Set target mới thao tác được. Mỗi nhân viên có target cá nhân riêng gồm doanh thu, UPT, ATV, CR. Target quản lý/cửa hàng sẽ tự lấy tổng target doanh thu và trung bình target UPT/ATV/CR của nhân viên trong cửa hàng.</p><form id="targetForm" class="grid four"><div class="field"><label>Nhân viên bán hàng</label><select name="user_id" required>${employeesOptions}</select></div><div class="field"><label>Tháng target</label><input class="input" type="month" name="target_month" value="${currentMonth}" required></div><div class="field"><label>Target doanh thu tháng</label><input class="input" type="number" name="target_revenue" min="0" required placeholder="VD: 80000000"></div><div class="field"><label>Target UPT</label><input class="input" type="number" step="0.01" name="target_upt" min="0" placeholder="VD: 2.50"></div><div class="field"><label>Target ATV</label><input class="input" type="number" name="target_atv" min="0" placeholder="VD: 1500000"></div><div class="field"><label>Target CR %</label><input class="input" type="number" step="0.01" name="target_cr" min="0" placeholder="VD: 35"></div><div class="field" style="grid-column:span 2"><label>Ghi chú target</label><input class="input" name="note" placeholder="VD: Target tháng 7 / target điều chỉnh"></div><div class="field" style="align-self:end"><button class="btn">Lưu target cá nhân</button></div></form></div>` : '';
  const dailyTargetForm = can('can_set_sales_targets') ? `<div class="card" style="margin-top:16px"><h3>Set target từng ngày - tổng cửa hàng</h3><p class="hint">Target ngày áp dụng cho tổng cửa hàng. Bảng tổng hợp sẽ so sánh doanh thu ngày, UPT, ATV, CR với target ngày và đổi màu đạt/chưa đạt.</p><form id="dailyTargetForm" class="grid four">${storeSelectTarget}<div class="field"><label>Ngày target</label><input class="input" type="date" name="target_date" value="${new Date().toISOString().slice(0,10)}" required></div><div class="field"><label>Target doanh thu ngày</label><input class="input" type="number" name="target_revenue" min="0" required placeholder="VD: 10000000"></div><div class="field"><label>Target UPT ngày</label><input class="input" type="number" step="0.01" name="target_upt" min="0" placeholder="VD: 2.44"></div><div class="field"><label>Target ATV ngày</label><input class="input" type="number" name="target_atv" min="0" placeholder="VD: 1500000"></div><div class="field"><label>Target CR ngày %</label><input class="input" type="number" step="0.01" name="target_cr" min="0" placeholder="VD: 35"></div><div class="field" style="grid-column:span 2"><label>Ghi chú target ngày</label><input class="input" name="note" placeholder="VD: Target cuối tuần / ngày sale"></div><div class="field" style="align-self:end"><button class="btn">Lưu target ngày</button></div></form></div>` : '';
  ...
  const summaryBlock = summary ? `<div class="card" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Tổng hợp doanh thu tháng ${esc(summary.month)} - ${esc(summary.store_name)}</h3></div><p class="hint">Bảng này cộng từng ngày đã nhập để theo dõi cửa hàng đạt target doanh thu, UPT, ATV, CR như thế nào. CR tính theo tổng bill / tổng khách cửa hàng.</p>${tableStoreSalesSummary(summary)}</div>` : '<div class="card" style="margin-top:16px"><h3>Tổng hợp doanh thu cửa hàng</h3><div class="empty">Tài khoản này chưa được cấp quyền xem tổng doanh thu cửa hàng</div></div>';
  ...
  $('#targetForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await api('/api/sales/targets', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(e.target))) }); toast('Đã lưu target cá nhân'); renderSales(); } catch (err) { toast(err.message, 'danger'); } });
  $('#dailyTargetForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await api('/api/sales/daily-targets', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(e.target))) }); toast('Đã lưu target ngày cửa hàng'); renderSales(); } catch (err) { toast(err.message, 'danger'); } });
