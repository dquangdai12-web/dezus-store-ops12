...
async function downloadExport(type) {
  try {
    const res = await fetch(`/api/export/${type}.xlsx`, { headers: { Authorization: `Bearer ${state.token}` } });
...
async function renderProductAnalytics(){
  ...
  const tabDefs=[['summary','Tổng hợp'],['sales','Chi tiết bán hàng'],['imports','Chi tiết nhập hàng'],['stock','Hàng tồn nhiều'],['slow','Hàng bán chậm']];if(isAdmin)tabDefs.push(['upload','Nhập file Sapo']);if(!isAdmin&&state.productAnalyticsTab==='upload')state.productAnalyticsTab='summary';
