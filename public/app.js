...
async function renderDailyReport() {
  ...
  $('#dailyReportViewDateFilter')?.addEventListener('change', e => { state.dailyReportDate = e.target.value; state.dailyReportEditMode = false; renderDailyReport(); });
  $('#dailyReportViewStoreFilter')?.addEventListener('change', e => { state.dailyReportStoreId = e.target.value; state.dailyReportEditMode = false; renderDailyReport(); });
  $('#dailyReportDataBtn')?.addEventListener('click', () => { state.dailyReportEditMode = true; renderDailyReport(); });
...
async function renderWeeklyReport() {
  ...
  const feedbackView = `<div class="card weekly-feedback-card" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Feedback tuần</h3></div>${hasFeedback ? `<div class="weekly-feedback-view"><div class="weekly-feedback-block"><b>Nhận xét tuần</b><div class="weekly-feedback-content">${esc(feedback.feedback || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Vấn đề phát sinh</b><div class="weekly-feedback-content">${esc(feedback.issues || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Việc cần làm tuần tới</b><div class="weekly-feedback-content">${esc(feedback.action_plan || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Feedback sản phẩm</b><div class="weekly-feedback-content">${esc(feedback.note || 'Chưa nhập')}</div></div></div>` : '<div class="empty">Chưa lưu feedback tuần này. Bấm <b>Nhập dữ liệu</b> ở phía trên để nhập/cập nhật.</div>'}</div>
  ...
  const weeklyDataAction = canEditFeedback ? (state.weeklyEditMode ? '<button class="btn" type="submit" form="weeklyFeedbackForm">Lưu báo cáo tuần</button>' : '<button class="btn" type="button" id="weeklyDataBtn">Nhập dữ liệu</button>') : '';
  shell(`<div class="card"><div class="toolbar"><div class="field"><label>Chọn ngày trong tuần</label><input class="input" id="weeklyDateFilter" type="date" value="${esc(data.week_start)}"></div>${storeFilter}${weeklyStatusFilter}<div class="field"><label>Tuần đang xem</label><div class="input readonly">${dOnly(data.week_start)} - ${dOnly(data.week_end)}</div></div><div style="align-self:end" class="row">${weeklyDataAction}${can('can_export') ? '<button class="btn secondary" id="weeklyExportBtn">Tải Excel báo cáo tuần</button>' : ''}</div></div><p class="hint">Chọn bất kỳ ngày trong tuần, hệ thống tự gom từ T2 đến CN. Mặc định chỉ hiển thị dữ liệu đã lưu. Bấm Nhập dữ liệu để mở form; khi đang nhập, nút sẽ đổi thành Lưu báo cáo tuần.</p></div>${kpis}<section class="grid two" style="margin-top:16px"><div class="card"><h3>Doanh thu theo ngày</h3>${tableWeeklyDays(data.days || [])}</div><div class="card"><h3>Doanh thu cá nhân</h3>${tableWeeklyEmployees(data.employees || [])}</div></section>${feedbackSection}`, 'Báo cáo tuần', `Tổng hợp tuần ${dOnly(data.week_start)} - ${dOnly(data.week_end)}`);
  ...
  $('#weeklyDataBtn')?.addEventListener('click', () => { state.weeklyEditMode = true; renderWeeklyReport(); });
