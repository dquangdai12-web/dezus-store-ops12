...
function tableWeeklyPromotions(rows) {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>CTKM</th><th>Số bill tham gia</th><th>Ghi chú</th></tr></thead><tbody>${rows.map(r => `<tr><td><b>${esc(r.name || '')}</b></td><td>${money(r.bill_count || 0)}</td><td>${esc(r.note || '')}</td></tr>`).join('')}</tbody></table></div>`;
}
