...
const state = {
  ...
  salesDailyDate: isoDateLocal(new Date()),
  salesStoreId: '',
  ...
  dailyReportEditMode: false,
  salesTargetEditMode: false,
  salesDailyTargetEditMode: false,
  salesDailyEntryMode: false,
  ...
  adminTransferUserId: null,
  adminTransferMonth: currentMonthLocal(),
...
const PERM_LABELS = {
  ...
  can_delete_users: 'Xóa vĩnh viễn',
...
function neutralTargetBadge(actual, target, suffix = '', decimals = 0) {
  actual = Number(actual || 0);
  target = Number(target || 0);
  if (!target) return '<span class="badge neutral-target-badge">Chưa set</span>';
  return `<span class="badge neutral-target-badge">${fmtKpi(actual, suffix, decimals)} / ${fmtKpi(target, suffix, decimals)}</span>`;
}

function smartTargetBadge(actual, target, suffix = '', decimals = 0) {
  actual = Number(actual || 0);
  target = Number(target || 0);
  if (!target) return '<span class="badge neutral-target-badge">Chưa set</span>';
  if (actual >= target) return `<span class="badge ok">${fmtKpi(actual, suffix, decimals)} / ${fmtKpi(target, suffix, decimals)}</span>`;
  return `<span class="badge neutral-target-badge">${fmtKpi(actual, suffix, decimals)} / ${fmtKpi(target, suffix, decimals)}</span>`;
}

function percentBadge(percent) {
  percent = Number(percent || 0);
  return `<span class="badge ok percent-target-badge"><b>${percent}%</b></span>`;
}
...
async function downloadExport(type) {
  try {
    let exportUrl = `/api/export/${type}.xlsx`;
    let downloadName = `${type}.xlsx`;
    if (type === 'performance') {
      const year = Number(state.reportYear || new Date().getFullYear());
      const month = Number(state.reportMonth || (new Date().getMonth() + 1));
      const storeId = state.reportStoreId || '';
      const qs = new URLSearchParams({ year: String(year), month: String(month) });
      if (storeId) qs.set('store_id', String(storeId));
      exportUrl += `?${qs.toString()}`;
      downloadName = `tong-hop-diem-${year}-${String(month).padStart(2,'0')}.xlsx`;
    }
    const res = await fetch(exportUrl, { headers: { Authorization: `Bearer ${state.token}` } });
...
function statusBadge(status) {
  const map = {
    ...
    overdue: ['Quá hạn dưới 4 tiếng', 'warn'],
    not_completed: ['Không hoàn thành', 'danger'],
    completed_on_time: ['Hoàn thành đúng hạn', 'ok'],
    completed_late: ['Hoàn thành trễ dưới 4 tiếng', 'warn'],
  };
...
function selectableStores(user = state.user) {
  const stores = Array.isArray(state.boot?.stores) ? state.boot.stores : [];
  if (user?.role === 'admin' || user?.role === 'office') return stores;
  const ids = Array.isArray(user?.store_ids) && user.store_ids.length
    ? user.store_ids.map(Number)
    : (user?.store_id ? [Number(user.store_id)] : []);
  return stores.filter(s => ids.includes(Number(s.id)));
}
...
function salesStaffInStore(storeId) {
  const sid = Number(storeId || 0);
  return state.boot.users.filter(u => u.role === 'employee' && (!sid || Number(u.store_id) === sid || (Array.isArray(u.store_ids) && u.store_ids.map(Number).includes(sid))));
}
...
async function render() {
  ...
  try {
    ...
    state.route = active;
    if (active === 'dashboard') return await renderDashboard();
    if (active === 'account') return renderAccount();
    if (active === 'tasks') return await renderTasks();
    if (active === 'violations') return await renderViolations();
    if (active === 'checklists') return await renderChecklists();
    ...
    if (active === 'bonuses') return await renderBonuses();
    if (active === 'reports') return await renderReports();
    if (active === 'admin') return await renderAdmin();
  } catch (err) {
    console.error('Render error:', err);
    const msg = err?.message || 'Không thể tải giao diện';
    if (typeof shell === 'function') shell(`<div class="card"><div class="empty"><b>Không thể tải trang</b><p>${esc(msg)}</p><button class="btn" id="retryRender" type="button">Tải lại</button></div></div>`, 'Có lỗi tải trang', 'Vui lòng thử tải lại');
    $('#retryRender')?.addEventListener('click', () => render());
  }
}
...
function tableLeaderboard(rows, showAmounts = false) {
  ...
  const moneyColsBody = r => showAmounts ? `<td class="sales-green-value sales-revenue-value"><b>${money(r.revenue || 0)}đ</b></td><td>${money(r.target || 0)}đ</td><td class="sales-green-value sales-timeline-value">${percentBadge(r.pace_percent || 0)}</td><td class="sales-green-value sales-daily-needed-value"><b>${money(r.daily_needed || 0)}đ</b></td>` : '';
  ...
  return `<div class="leaderboard-premium sales-leaderboard-premium">${podium}</div><div class="table-wrap leaderboard-wrap"><table class="leaderboard-table"><thead><tr><th>Top</th><th>Nhân viên</th><th>Cửa hàng</th><th>% đạt target</th>${moneyColsHead}${revenueShareHead}<th>Bill</th><th>Món</th><th>UPT</th><th>ATV</th><th>ASP</th><th>GUESTS</th></tr></thead><tbody>${rows.map(r => `<tr><td><span class="badge dark">Top ${r.rank}</span></td><td class="employee-name-cell"><b>${userDisplayName(r)}</b></td><td class="store-name-cell">${esc(r.store_name || '')}</td><td class="sales-green-value sales-target-percent">${percentBadge(r.achievement_percent || 0)}</td>${moneyColsBody(r)}${revenueShareBody(r)}<td>${money(r.bill_count || 0)}</td><td>${money(r.item_count || 0)}</td><td class="metric-badge-cell">${smartTargetBadge(r.upt || 0, r.target_upt || 0, '', 2)}</td><td class="metric-badge-cell">${smartTargetBadge(r.atv || 0, r.target_atv || 0, 'đ')}</td><td>${money(r.asp || 0)}đ</td><td>${Math.round(r.guests_percent || 0)}%</td></tr>`).join('')}</tbody></table></div>`;
}
...
function tableStoreSalesSummary(summary) {
  ...
  const kpis = `<div class="grid four dash-kpis sales-summary-grid" style="margin-bottom:14px"><div class="card kpi sales-kpi kpi-revenue"><div class="label">Doanh thu thực đạt</div><div class="num">${money(t.revenue || 0)}đ</div><div class="hint">Target tháng: ${money(summary.monthly_target || 0)}đ • ${percentBadge(t.achievement_percent || 0)}</div></div><div class="card kpi sales-kpi kpi-timeline"><div class="label">Timeline dự kiến</div><div class="num">${Number(t.pace_percent || 0)}%</div><div class="hint">Dự kiến về: ${money(t.projected_revenue || 0)}đ</div></div><div class="card kpi sales-kpi kpi-dailyneed"><div class="label">Cần bán/ngày</div><div class="num">${money(t.daily_needed || 0)}đ</div><div class="hint">Còn ${money(t.days_remaining || 0)} ngày để về 100%</div></div><div class="card kpi sales-kpi kpi-dailytarget"><div class="label">Target ngày đã set</div><div class="num">${money(t.daily_target || 0)}đ</div><div class="hint">Theo tổng target ngày trong tháng • ${percentBadge(t.daily_achievement_percent || 0)}</div></div><div class="card kpi sales-kpi kpi-upt"><div class="label">UPT</div><div class="num">${fmt2(t.upt || 0)}</div><div class="hint">${smartTargetBadge(t.upt || 0, t.target_upt || 0, '', 2)}</div></div><div class="card kpi sales-kpi kpi-atv"><div class="label">ATV</div><div class="num">${money(t.atv || 0)}đ</div><div class="hint">${smartTargetBadge(t.atv || 0, t.target_atv || 0, 'đ')}</div></div><div class="card kpi sales-kpi kpi-asp"><div class="label">ASP</div><div class="num">${money(t.asp || 0)}đ</div><div class="hint">Doanh thu / số món</div></div><div class="card kpi sales-kpi kpi-cr"><div class="label">CR</div><div class="num">${Number(t.cr || 0)}%</div><div class="hint">${smartTargetBadge(t.cr || 0, t.target_cr || 0, '%', 2)}</div></div><div class="card kpi sales-kpi kpi-new"><div class="label">Khách mới</div><div class="num">${money(t.customer_new_count || 0)}</div><div class="hint">TF khách mới trong tháng</div></div><div class="card kpi sales-kpi kpi-old"><div class="label">Khách cũ</div><div class="num">${money(t.customer_old_count || 0)}</div><div class="hint">TF khách cũ trong tháng</div></div><div class="card kpi sales-kpi kpi-totalguest"><div class="label">Lượt khách tổng</div><div class="num">${money(t.customer_count || 0)}</div><div class="hint">Khách mới + khách cũ</div></div></div>`;
  const rows = summary.rows.map(r => `<tr><td><b>${dOnly(r.sale_date)}</b></td><td class="sales-green-value sales-revenue-value"><b>${money(r.revenue || 0)}</b></td><td>${money(r.daily_target || 0)}</td><td class="sales-green-value sales-target-percent">${percentBadge(r.daily_achievement_percent || 0)}</td><td>${money(r.bill_count || 0)}</td><td>${money(r.item_count || 0)}</td><td>${money(r.customer_new_count || 0)}</td><td>${money(r.customer_old_count || 0)}</td><td>${money(r.customer_count || 0)}</td><td>${smartTargetBadge(r.upt || 0, r.daily_target_upt || r.target_upt || 0, '', 2)}</td><td>${smartTargetBadge(r.atv || 0, r.daily_target_atv || r.target_atv || 0, 'đ')}</td><td>${money(r.asp || 0)}đ</td><td>${smartTargetBadge(r.cr || 0, r.daily_target_cr || r.target_cr || 0, '%', 2)}</td><td class="sales-green-value sales-target-percent">${percentBadge(r.achievement_percent || 0)}</td><td>${esc(r.note || r.daily_target_note || '')}</td></tr>`).join('');
...
async function downloadWeeklyReport() {
  ...
  try {
    ...
    const res = await fetch(`/api/weekly-report/export.xlsx?week_start=${encodeURIComponent(weekStart)}${storeId ? `&store_id=${encodeURIComponent(storeId)}` : ''}&user_status=${encodeURIComponent(userStatus)}`, { headers: { Authorization: `Bearer ${state.token}` } });
    if (!res.ok) throw new Error('Không tải được báo cáo tuần');
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `bao-cao-tuan-${weekStart}.xlsx`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
...
async function renderDailyReport() {
  ...
  const dailyEditMode = !!state.dailyReportEditMode;
  ...
  const dailyViewStoreFilter = isAllStoreUser() ? `<div class="field"><label>Cửa hàng</label><select class="input" id="dailyReportViewStoreFilter">${storeOptions}</select></div>` : `<div class="field"><label>Cửa hàng</label><div class="input readonly">${esc(data.store_name || '')}</div></div>`;
  const dailyEntryControl = ''; // v4.177: báo cáo ngày hiển thị form nhập sẵn như trước

  const form = `<form id="dailyReportForm">
    <div class="card daily-report-head"><div class="toolbar"><div class="field"><label>Ngày báo cáo</label><input class="input" id="dailyReportDateFilter" name="report_date" type="date" value="${esc(data.report_date || reportDate)}" required></div>${storeFilter}${canEdit ? '<div style="margin-left:auto;align-self:end"><button class="btn" type="submit">Lưu báo cáo ngày</button></div>' : ''}</div><div class="daily-customer-shifts"><div class="daily-shift-customer-card"><div class="section-title"><h3>Ca sáng</h3></div><div class="grid two"><div class="field"><label>Khách mới</label><input class="input daily-customer-shift" type="text" inputmode="numeric" data-number-format name="customer_new_morning" value="${money(data.customer_new_morning || 0)}" ${canEdit ? '' : 'disabled'}></div><div class="field"><label>Khách cũ</label><input class="input daily-customer-shift" type="text" inputmode="numeric" data-number-format name="customer_old_morning" value="${money(data.customer_old_morning || 0)}" ${canEdit ? '' : 'disabled'}></div></div></div><div class="daily-shift-customer-card"><div class="section-title"><h3>Ca chiều</h3></div><div class="grid two"><div class="field"><label>Khách mới</label><input class="input daily-customer-shift" type="text" inputmode="numeric" data-number-format name="customer_new_afternoon" value="${money(data.customer_new_afternoon || 0)}" ${canEdit ? '' : 'disabled'}></div><div class="field"><label>Khách cũ</label><input class="input daily-customer-shift" type="text" inputmode="numeric" data-number-format name="customer_old_afternoon" value="${money(data.customer_old_afternoon || 0)}" ${canEdit ? '' : 'disabled'}></div></div></div><div class="daily-shift-total-card"><div class="field"><label>Khách mới tổng</label><input class="input readonly" type="text" inputmode="numeric" data-number-format name="customer_new_count" value="${money(data.customer_new_count || 0)}" readonly></div><div class="field"><label>Khách cũ tổng</label><input class="input readonly" type="text" inputmode="numeric" data-number-format name="customer_old_count" value="${money(data.customer_old_count || 0)}" readonly></div><div class="field"><label>Lượt khách tổng</label><input class="input readonly" type="text" inputmode="numeric" data-number-format name="customer_count" value="${money(customerTotalValue(data.customer_new_count || 0, data.customer_old_count || 0, data.customer_count || 0))}" readonly></div></div></div></div>
    ...
    ${canEdit ? '' : '<div class="empty" style="margin-top:16px">Tài khoản này chỉ được xem báo cáo ngày.</div>'}
  </form>`;
  shell(`${dailyEntryControl}${form}${dailyReportOverviewBox(data)}`, 'Báo cáo ngày', `Nhập báo cáo ngày ${dOnly(data.report_date || reportDate)} - ${esc(data.store_name || '')}`);
  const dailyFormEl = $('#dailyReportForm');
  const syncDailyShiftCustomers = () => {
    if (!dailyFormEl) return;
    const nm = cleanNumberInput($('[name="customer_new_morning"]', dailyFormEl)?.value);
    const om = cleanNumberInput($('[name="customer_old_morning"]', dailyFormEl)?.value);
    const na = cleanNumberInput($('[name="customer_new_afternoon"]', dailyFormEl)?.value);
    const oa = cleanNumberInput($('[name="customer_old_afternoon"]', dailyFormEl)?.value);
    const newTotal = nm + na;
    const oldTotal = om + oa;
    const newEl = $('[name="customer_new_count"]', dailyFormEl);
    const oldEl = $('[name="customer_old_count"]', dailyFormEl);
    const totalEl = $('[name="customer_count"]', dailyFormEl);
    if (newEl) newEl.value = money(newTotal);
    if (oldEl) oldEl.value = money(oldTotal);
    if (totalEl) totalEl.value = money(newTotal + oldTotal);
  };
  $$('.daily-customer-shift', dailyFormEl).forEach(el => el.addEventListener('input', syncDailyShiftCustomers));
  syncDailyShiftCustomers();
  ...
  $('#dailyReportForm')?.addEventListener('submit', async e => {
    ...
    const payload = { store_id: fd.get('store_id') || data.store_id, report_date: fd.get('report_date'), customer_new_morning: cleanNumberInput(fd.get('customer_new_morning')), customer_old_morning: cleanNumberInput(fd.get('customer_old_morning')), customer_new_afternoon: cleanNumberInput(fd.get('customer_new_afternoon')), customer_old_afternoon: cleanNumberInput(fd.get('customer_old_afternoon')), customer_new_count: cleanNumberInput(fd.get('customer_new_count')), customer_old_count: cleanNumberInput(fd.get('customer_old_count')), customer_count: cleanNumberInput(fd.get('customer_count')), store_situation_morning: morningSituation, store_situation_evening: eveningSituation, store_note: [morningSituation ? `Ca sáng: ${morningSituation}` : '', eveningSituation ? `Ca tối: ${eveningSituation}` : ''].filter(Boolean).join('\n'), sales_entries, missing_size_items, product_feedback_items };
    try {
      ...
      state.dailyReportEditMode = false;
      renderDailyReport();
    } catch (err) { toast(err.message, 'danger'); }
  });
}
...
async function renderWeeklyReport() {
  ...
  const weeklyEditing = !!state.weeklyEditMode;
  const feedbackView = `<div class="card weekly-feedback-card" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Feedback tuần</h3>${canEditFeedback ? '<button type="button" class="btn secondary" id="weeklyEditFeedbackBtn">Sửa feedback</button>' : ''}</div>${hasFeedback ? `<div class="weekly-feedback-view"><div class="weekly-feedback-block"><b>Nhận xét tuần</b><div class="weekly-feedback-content">${esc(feedback.feedback || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Vấn đề phát sinh</b><div class="weekly-feedback-content">${esc(feedback.issues || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Việc cần làm tuần tới</b><div class="weekly-feedback-content">${esc(feedback.action_plan || 'Chưa nhập')}</div></div><div class="weekly-feedback-block"><b>Feedback sản phẩm</b><div class="weekly-feedback-content">${esc(feedback.note || 'Chưa nhập')}</div></div></div>` : '<div class="empty">Chưa lưu feedback tuần này.</div>'}</div>
  ...
  $('#weeklyFeedbackForm')?.addEventListener('submit', async e => {
    ...
    try { await api('/api/weekly-report', { method: 'POST', body: JSON.stringify(payload) }); toast('Đã lưu feedback tuần'); state.weeklyEditMode = false; renderWeeklyReport(); } catch (err) { toast(err.message, 'danger'); }
  });
}
...
async function renderTasks() {
  ...
  const taskMonthParts = currentTaskMonth.split('-').map(Number);
  const taskMonthEnd = isoDateLocal(new Date(taskMonthParts[0], taskMonthParts[1], 0));
  const bulkStoreOptions = `<option value="all">Tất cả cửa hàng</option>${(state.boot?.stores || []).map(st => `<option value="${st.id}">${esc(st.name)}</option>`).join('')}`;
  const bulkUserOptions = `<option value="all">Tất cả nhân viên</option>${employees().map(u => `<option value="${u.id}">${esc(u.full_name)}${u.store_name ? ` • ${esc(u.store_name)}` : ''}</option>`).join('')}`;
  const bulkCategoryOptions = `<option value="all">Tất cả nhóm công việc</option>${TASK_CATEGORY_OPTIONS.map(([v,l]) => `<option value="${v}">${esc(l)}</option>`).join('')}`;
  const adminBulkDeletePanel = state.user.role === 'admin' ? `
    <div class="card task-bulk-delete-card task-bulk-delete-collapsed" style="margin-top:16px">
      <div class="section-title task-bulk-delete-head">
        <div><p class="eyebrow">Admin</p><h3>Xóa dữ liệu phân việc</h3><p class="hint">Dùng khi giao nhầm nhiều công việc. Có thể lọc theo cửa hàng, ngày, nhân viên, nhóm và trạng thái.</p></div>
        <button type="button" class="btn secondary" id="taskBulkDeleteToggle">Mở công cụ xóa</button>
      </div>
      <div id="taskBulkDeleteBody" hidden>
        <div class="task-bulk-delete-warning"><b>Chỉ Admin được thao tác.</b><span>Xóa dữ liệu sẽ ảnh hưởng trực tiếp đến khối lượng và điểm công việc. Hãy kiểm tra số lượng trước khi xác nhận.</span></div>
        <div class="grid three task-bulk-delete-grid">
          <div class="field"><label>Cửa hàng</label><select id="taskBulkStore">${bulkStoreOptions}</select></div>
          <div class="field"><label>Từ ngày</label><input id="taskBulkFrom" type="date" value="${currentTaskMonth}-01"></div>
          <div class="field"><label>Đến ngày</label><input id="taskBulkTo" type="date" value="${taskMonthEnd}"></div>
          <div class="field"><label>Nhân viên</label><select id="taskBulkUser">${bulkUserOptions}</select></div>
          <div class="field"><label>Nhóm công việc</label><select id="taskBulkCategory">${bulkCategoryOptions}</select></div>
          <div class="field task-bulk-status-field"><label>Trạng thái cần xóa</label><div class="task-bulk-statuses">
            <label><input type="checkbox" name="taskBulkStatus" value="assigned" checked><span>Đang mở</span></label>
            <label><input type="checkbox" name="taskBulkStatus" value="overdue" checked><span>Quá hạn</span></label>
            <label><input type="checkbox" name="taskBulkStatus" value="not_completed" checked><span>Không hoàn thành</span></label>
            <label><input type="checkbox" name="taskBulkStatus" value="completed_on_time" checked><span>Hoàn thành đúng hạn</span></label>
            <label><input type="checkbox" name="taskBulkStatus" value="completed_late" checked><span>Hoàn thành trễ</span></label>
          </div></div>
        </div>
        <div class="task-bulk-delete-result" id="taskBulkDeleteResult"><span>Chưa kiểm tra dữ liệu.</span></div>
        <div class="task-bulk-delete-actions">
          <button type="button" class="btn secondary" id="taskBulkPreviewBtn">Kiểm tra dữ liệu</button>
          <button type="button" class="btn danger" id="taskBulkDeleteFilteredBtn">Xóa vùng đã chọn</button>
          <button type="button" class="btn danger task-delete-all-btn" id="taskDeleteAllBtn">Xóa toàn bộ dữ liệu phân việc</button>
        </div>
      </div>
    </div>` : '';
  ...
  taskBulkDeleteToggle?.addEventListener('click', () => {
    if (!taskBulkDeleteBody) return;
    const willOpen = taskBulkDeleteBody.hidden;
    taskBulkDeleteBody.hidden = !willOpen;
    taskBulkDeleteToggle.textContent = willOpen ? 'Đóng công cụ xóa' : 'Mở công cụ xóa';
    taskBulkDeleteToggle.closest('.task-bulk-delete-card')?.classList.toggle('task-bulk-delete-collapsed', !willOpen);
  });
  ...
  $$('.taskExceptionTab').forEach(btn => btn.addEventListener('click', () => {
    ...
    const box = $('#taskExceptionList');
    if (box) box.innerHTML = rows.map(t => taskCard(t)).join('') || '<div class="empty">Không có công việc trong nhóm này</div>';
    $$('.completeForm', box || document).forEach(f => f.addEventListener('submit', submitCompleteTask));
    $$('.editTaskBtn', box || document).forEach(x => x.addEventListener('click', openTaskEditor));
    $$('.deleteTaskBtn', box || document).forEach(x => x.addEventListener('click', deleteTask));
  }));
...
function taskBulkDeletePayload(dryRun = false) {
  const statuses = $$('input[name="taskBulkStatus"]:checked').map(x => x.value);
  return {
    mode: 'filtered',
    store_id: $('#taskBulkStore')?.value || 'all',
    user_id: $('#taskBulkUser')?.value || 'all',
    category: $('#taskBulkCategory')?.value || 'all',
    from_date: $('#taskBulkFrom')?.value || '',
    to_date: $('#taskBulkTo')?.value || '',
    statuses,
    dry_run: dryRun ? 1 : 0
  };
}

function setTaskBulkResult(message, tone = '') {
  const box = $('#taskBulkDeleteResult');
  if (!box) return;
  box.className = `task-bulk-delete-result${tone ? ` ${tone}` : ''}`;
  box.innerHTML = message;
}

async function previewTaskBulkDelete() {
  const payload = taskBulkDeletePayload(true);
  if (!payload.statuses.length) return toast('Vui lòng chọn ít nhất một trạng thái cần xóa', 'danger');
  const btn = $('#taskBulkPreviewBtn');
  if (btn) { btn.disabled = true; btn.textContent = 'Đang kiểm tra...'; }
  try {
    const res = await api('/api/tasks/bulk-delete', { method:'POST', body:JSON.stringify(payload) });
    const count = Number(res.matched_assignments || 0);
    setTaskBulkResult(count
      ? `<b>${count} dòng phân việc</b> phù hợp • ảnh hưởng <b>${Number(res.affected_tasks || 0)} công việc</b> • ${Number(res.removable_tasks || 0)} công việc sẽ được xóa hoàn toàn nếu không còn người nhận khác.`
      : '<b>Không có dữ liệu phù hợp</b> với vùng đang chọn.', count ? 'warning' : 'ok');
  } catch (err) {
    setTaskBulkResult(esc(err.message), 'danger');
    toast(err.message, 'danger');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'Kiểm tra dữ liệu'; }
  }
}

async function deleteTaskBulkFiltered() {
  const payload = taskBulkDeletePayload(true);
  if (!payload.statuses.length) return toast('Vui lòng chọn ít nhất một trạng thái cần xóa', 'danger');
  const btn = $('#taskBulkDeleteFilteredBtn');
  if (btn) btn.disabled = true;
  try {
    const preview = await api('/api/tasks/bulk-delete', { method:'POST', body:JSON.stringify(payload) });
    const count = Number(preview.matched_assignments || 0);
    if (!count) {
      setTaskBulkResult('<b>Không có dữ liệu phù hợp</b> với vùng đang chọn.', 'ok');
      return toast('Không có dữ liệu để xóa');
    }
...
Dữ liệu sẽ được chuyển vào Thùng rác và Admin có thể khôi phục lại. Tiếp tục?`);
    ...
    const finalPayload = { ...payload, dry_run: 0 };
    const res = await api('/api/tasks/bulk-delete', { method:'POST', body:JSON.stringify(finalPayload) });
    toast(`Đã xóa ${Number(res.deleted_assignments || 0)} dòng phân việc`);
    await renderTasks();
  } catch (err) {
    toast(err.message, 'danger');
  } finally {
...
async function deleteAllTaskData() {
  if (state.user?.role !== 'admin') return toast('Chỉ Admin được xóa toàn bộ dữ liệu', 'danger');
  const btn = $('#taskDeleteAllBtn');
  if (btn) btn.disabled = true;
  try {
    const preview = await api('/api/tasks/bulk-delete', { method:'POST', body:JSON.stringify({ mode:'all', dry_run:1 }) });
    const count = Number(preview.matched_assignments || 0);
    const taskCount = Number(preview.affected_tasks || 0);
    if (!taskCount && !count) return toast('Hiện không có dữ liệu phân việc để xóa');
    if (!confirm(`CẢNH BÁO: Bạn đang chọn XÓA TOÀN BỘ dữ liệu phân việc của tất cả cửa hàng và tất cả thời gian.
...
Dữ liệu sẽ được chuyển vào Thùng rác và có thể khôi phục.`)) return;
    ...
    toast(`Đã xóa toàn bộ: ${Number(res.deleted_tasks || 0)} công việc`);
    await renderTasks();
  } catch (err) {
    toast(err.message, 'danger');
  } finally {
...
function taskCard(t) {
  ...
  const isLate = String(t.status || '') === 'overdue' || String(t.status || '') === 'completed_late';
...
async function renderViolations() {
  ...
  shell(`${form}<div class="card" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Danh sách vi phạm</h3>${can('can_export') ? '<button class="btn secondary" data-export="violations">Tải Excel</button>' : ''}</div>${list}</div>`, 'Vi phạm', '');
...
async function renderChecklists() {
  ...
  const exportBtn = can('can_export') ? '<button class="btn secondary" data-export="assessments">Tải Excel</button>' : '';
...
async function renderGGNV() {
  ...
  const list = `<div class="card" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Danh sách GGNV</h3>${canViewAll ? `<div class="field"><label>Cửa hàng xem</label><select class="input" id="ggnvListStoreFilter">${listStoreOptions}</select></div>` : ''}</div><div class="table-wrap"><table><thead><tr><th>Ngày</th><th>Nhân viên</th><th>Cửa hàng</th><th>Số đơn</th><th>Giá trị đơn hàng</th><th>Mức giảm</th><th>Chứng từ</th>${canViewAll ? '<th>Thao tác</th>' : ''}</tr></thead><tbody>${rows.length ? rows.map(r => `<tr><td><b>${dOnly(r.sale_date)}</b></td><td><b>${esc(r.employee_name || '')}</b></td><td>${esc(r.store_name || '')}</td><td>${esc(r.order_number || '')}</td><td><b>${money(r.order_value || 0)}đ</b></td><td>${Number(r.discount_rate || 0)}%</td><td>${r.evidence_path ? renderFiles(r.evidence_path) : '-'}</td>${canViewAll ? `<td><button class="btn danger small ggnvDeleteBtn" type="button" data-id="${r.id}" data-order="${esc(r.order_number || '')}">Xóa</button></td>` : ''}</tr>`).join('') : `<tr><td colspan="${canViewAll ? 8 : 7}"><div class="empty">Chưa có dữ liệu GGNV</div></td></tr>`}</tbody></table></div></div>`;
  ...
  $$('.ggnvDeleteBtn').forEach(btn => btn.onclick = async () => {
    const label = btn.dataset.order ? `đơn ${btn.dataset.order}` : 'đơn GGNV này';
    if (!confirm(`Xóa ${label}? Thao tác này không thể hoàn tác.`)) return;
    try { await api(`/api/ggnv/${btn.dataset.id}`, { method:'DELETE' }); toast('Đã xóa đơn GGNV'); renderGGNV(); } catch (err) { toast(err.message, 'danger'); }
  });
}
...
async function renderLoyalty() {
  ...
  $$('.loyaltyDeleteBtn').forEach(btn => btn.onclick = async () => {
    const invoice = btn.dataset.invoice ? ` ${btn.dataset.invoice}` : '';
    const approvedNote = btn.dataset.status === 'approved' ? ' Đơn đã duyệt sẽ được gỡ khỏi doanh thu đã cộng.' : '';
    if (!confirm(`Xóa đơn Loyalty${invoice}?${approvedNote} Thao tác này không thể hoàn tác.`)) return;
    try { await api(`/api/loyalty/${btn.dataset.id}`, { method:'DELETE' }); toast('Đã xóa đơn Loyalty'); renderLoyalty(); } catch (err) { toast(err.message, 'danger'); }
  });
...
async function renderSales() {
  ...
  const targetMonth = state.salesTargetMonth || currentMonth;
  state.salesTargetMonth = targetMonth;
  ...
  const dailyDate = state.salesDailyDate || isoDateLocal(new Date());
  state.salesDailyDate = dailyDate;
  const [data, summary, dailyStaffRes, targetStaffRes] = await Promise.all([
    ...
    defaultStoreId ? api(`/api/store-staff?store_id=${encodeURIComponent(defaultStoreId)}&date=${encodeURIComponent(dailyDate)}`).catch(() => null) : Promise.resolve({ employees: [] }),
    defaultStoreId ? api(`/api/store-staff?store_id=${encodeURIComponent(defaultStoreId)}&month=${encodeURIComponent(targetMonth)}`).catch(() => null) : Promise.resolve({ employees: [] })
  ]);
  ...
  const dailySalesStaff = Array.isArray(dailyStaffRes?.employees) ? dailyStaffRes.employees : salesStaff;
  const targetSalesStaff = Array.isArray(targetStaffRes?.employees) ? targetStaffRes.employees : salesStaff;
  const employeesOptions = salesStaff.map(u => `<option value="${u.id}">${esc(u.full_name)} - ${esc(u.store_name || '')}</option>`).join('');
  const selectedTargetStoreName = salesStoreChoices.find(x => Number(x.id) === Number(defaultStoreId))?.name || storeName(defaultStoreId) || '';
  const targetEmployeeChecks = targetSalesStaff.map(u => {
    const transferHint = Number(u.store_id) && Number(u.store_id) !== Number(defaultStoreId) ? ' • luân chuyển' : '';
    return `<label class="target-employee-check"><input type="checkbox" name="user_ids" value="${u.id}"><span><b>${esc(u.full_name)}</b><small>${esc(selectedTargetStoreName)} • ${esc(targetMonth)}${transferHint}</small></span></label>`;
  }).join('') || '<div class="empty">Chưa có nhân viên được phân công/điều chuyển tới cửa hàng này trong tháng target</div>';
  ...
  const targetStoreSelect = salesStoreChoices.length > 1 ? `<div class="field"><label>Cửa hàng áp dụng Target</label><select class="input" id="salesTargetStoreFilter" name="store_id">${storeOptions}</select></div>` : `<input type="hidden" name="store_id" value="${esc(defaultStoreId)}">`;
  ...
  const summaryBlock = summary ? `<div class="card sales-summary-shell" style="margin-top:16px"><div class="toolbar"><h3 style="margin-right:auto">Tổng hợp doanh thu tháng ${esc(summary.month)} - ${esc(summary.store_name)}</h3></div><p class="hint">Bảng này cộng từng ngày đã nhập để theo dõi doanh thu ngày; UPT, ATV, ASP, CR mặc định so với target tháng. Timeline dự kiến tính theo tốc độ doanh thu hiện tại và số ngày còn lại trong kỳ.</p>${tableStoreSalesSummary(summary)}</div>` : '<div class="card" style="margin-top:16px"><h3>Tổng hợp doanh thu cửa hàng</h3><div class="empty">Tài khoản này chưa được cấp quyền xem tổng doanh thu cửa hàng</div></div>';
  shell(`${targetForm}${dailyTargetForm}${salesForm}<div class="card" style="margin-top:16px">${monthFilter}<div class="toolbar"><h3 style="margin-right:auto">Bảng doanh thu thực đạt ${state.leaderboardPeriod === 'month' ? 'tháng' : state.leaderboardPeriod === 'quarter' ? 'quý' : 'năm'}</h3>${tabs}${can('can_export') ? '<button class="btn secondary" data-export="sales">Tải Excel doanh thu</button>' : ''}</div>${tableLeaderboard(data.leaderboard, true)}</div>${summaryBlock}`, 'Doanh thu', '');
  $('#salesTargetDataBtn')?.addEventListener('click', () => { state.salesTargetEditMode = true; renderSales(); });
  $('#salesDailyTargetDataBtn')?.addEventListener('click', () => { state.salesDailyTargetEditMode = true; renderSales(); });
  $('#salesDailyDataBtn')?.addEventListener('click', () => { state.salesDailyEntryMode = true; renderSales(); });
  $('#dailySalesForm [name="sale_date"]')?.addEventListener('change', e => { state.salesDailyDate = e.target.value || isoDateLocal(new Date()); renderSales(); });
  ...
  $('#salesTargetStoreFilter')?.addEventListener('change', e => { state.salesStoreId = e.target.value; renderSales(); });
  $('#salesTargetMonthInput')?.addEventListener('change', e => { state.salesTargetMonth = e.target.value || currentMonth; renderSales(); });
  ...
  $('#targetForm')?.addEventListener('submit', async e => {
    ...
    state.salesTargetMonth = payload.target_month || state.salesTargetMonth;
    ...
    try {
      ...
      state.salesTargetEditMode = false;
      renderSales();
    } catch (err) { toast(err.message, 'danger'); }
  });
  $('#dailyTargetForm')?.addEventListener('submit', async e => {
    ...
    try {
      ...
      state.salesDailyTargetEditMode = false;
      renderSales();
    } catch (err) { toast(err.message, 'danger'); }
  });
  ...
  $('#dailySalesForm')?.addEventListener('submit', async e => {
    ...
    try { await api('/api/sales/daily', { method: 'POST', body: JSON.stringify(payload) }); toast('Đã lưu doanh thu ngày'); state.salesDailyEntryMode = false; renderSales(); } catch (err) { toast(err.message, 'danger'); }
  });
}
...
function productAnalyticsSortRows(rows, mode='speed') {
  ...
  const configs={
    ...
    restock:{default:null,priority_desc:['restock_score',-1],qty_desc:['restock_qty',-1],qty_asc:['restock_qty',1],sold_desc:['sold_qty',-1],sold_asc:['sold_qty',1],velocity_desc:['velocity',-1],velocity_asc:['velocity',1],stock_desc:['stock_qty',-1],stock_asc:['stock_qty',1],cover_desc:['days_cover',-1],cover_asc:['days_cover',1],arrival_desc:['arrival_date',-1,'text'],arrival_asc:['arrival_date',1,'text'],name_asc:['product_name',1,'text'],name_desc:['product_name',-1,'text']},
...
function productAnalyticsSortControl(mode='speed') {
  ...
  const metric={
    ...
    restock:[['priority_desc','Ưu tiên tái cao nhất'],['qty_desc','SL đề xuất: lớn → nhỏ'],['qty_asc','SL đề xuất: nhỏ → lớn'],['sold_desc','Tổng bán: lớn → nhỏ'],['sold_asc','Tổng bán: nhỏ → lớn'],['velocity_desc','Tốc độ: lớn → nhỏ'],['velocity_asc','Tốc độ: nhỏ → lớn'],['stock_desc','Tồn: lớn → nhỏ'],['stock_asc','Tồn: nhỏ → lớn'],['cover_asc','Days of Cover: thấp → cao'],['cover_desc','Days of Cover: cao → thấp'],['arrival_desc','Ngày về: mới → cũ'],['arrival_asc','Ngày về: cũ → mới']],
...
function productAnalyticsTable(rows, mode='speed') {
  ...
  if(mode==='sales')return `<div class="table-wrap product-analytics-table"><table><thead><tr><th>STT</th>${H('Tên sản phẩm','product_name','text')}${H('Tồn đầu kỳ','opening_qty')}${H('Bán POS','pos_qty')}${H('Bán Online','online_qty')}${H('Tổng bán','sold_qty')}${H('Tổng nhập','import_qty')}${H('Tổng xuất','export_qty')}${H('Tồn cuối kỳ','stock_qty')}${H('TĐ POS/ngày','pos_velocity')}${H('TĐ Online/ngày','online_velocity')}${H('TĐ tổng/ngày','velocity')}</tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${rank(i)}</td><td><b>${esc(r.product_name||'')}</b></td><td>${money(r.opening_qty||0)}</td><td><b>${money(r.pos_qty||0)}</b></td><td><b>${money(r.online_qty||0)}</b></td><td><b>${money(r.sold_qty||0)}</b></td><td><b>${money(r.import_qty||0)}</b></td><td>${money(r.export_qty||0)}</td><td><b>${money(r.stock_qty||0)}</b></td><td>${fmt2(r.pos_velocity||0)}</td><td>${fmt2(r.online_velocity||0)}</td><td><b>${fmt2(r.velocity||0)}</b></td></tr>`).join('')}</tbody></table></div>`;

  if(mode==='imports')return `<div class="table-wrap product-analytics-table"><table><thead><tr><th>STT</th>${H('Tên sản phẩm','product_name','text')}${H('Ngày nhập gần nhất','arrival_date','text')}${H('Số ngày bán','selling_days')}${H('Hàng đầu kỳ','opening_qty')}${H('Tổng nhập','import_qty')}${H('Tổng xuất','export_qty')}${H('Bán POS','pos_qty')}${H('Bán Online','online_qty')}${unknownHead}${H('Tổng bán','sold_qty')}${H('Tồn cuối kỳ','stock_qty')}${H('Tốc độ bán/ngày','velocity')}${H('Ngày còn lại','remaining_days')}${H('Có thể bán thêm','potential_sell_qty')}${H('Miss sale ƯT','miss_sale_qty')}${H('Dự trữ đề xuất','reserve_recommended')}${H('Days of Cover','days_cover')}</tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${rank(i)}</td><td><b>${esc(r.product_name||'')}</b></td><td>${r.arrival_date?dOnly(r.arrival_date):'-'}</td><td>${r.selling_days??r.days_since_arrival??'-'}</td><td>${money(r.opening_qty||0)}</td><td><b>${money(r.import_qty||0)}</b></td><td>${money(r.export_qty||0)}</td><td>${money(r.pos_qty||0)}</td><td>${money(r.online_qty||0)}</td>${unknownCell(r)}<td>${money(r.sold_qty||0)}</td><td><b>${money(r.stock_qty||0)}</b></td><td>${fmt2(r.velocity||0)}</td><td>${money(r.remaining_days||0)}</td><td><b>${money(r.potential_sell_qty??r.sellable_qty??0)}</b></td><td><b>${money(r.miss_sale_qty||0)}</b></td><td>${money(r.reserve_recommended||0)}</td><td>${r.days_cover==null?'-':`${r.days_cover} ngày`}</td></tr>`).join('')}</tbody></table></div>`;

  if(mode==='restock'){
    const priorityBadge=r=>{const cls=r.restock_priority==='urgent'?'danger':r.restock_priority==='high'?'warn':'dark';return `<span class="badge ${cls}">${esc(r.restock_priority_label||'Đề xuất')}</span>`;};
    return `<div class="table-wrap product-analytics-table pa-restock-table"><table><thead><tr><th>STT</th>${H('Mức độ','restock_score')}${H('Tên sản phẩm / mã','product_name','text')}${H('Ngày về hàng','arrival_date','text')}${H('Số ngày từ khi về','days_since_arrival')}${H('SL bán','sold_qty')}${H('Tồn hiện tại','stock_qty')}${H('Tốc độ bán/ngày','velocity')}${H('Tỷ lệ đã bán','sell_through_pct')}${H('Days of Cover','days_cover')}${H('Mốc dự trữ','restock_target_days')}${H('SL đề xuất tái','restock_qty')}<th>Lý do</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${rank(i)}</td><td>${priorityBadge(r)}</td><td><b>${esc(r.product_name||'')}</b></td><td>${r.arrival_date?dOnly(r.arrival_date):'-'}</td><td>${r.days_since_arrival==null?'-':`${r.days_since_arrival} ngày`}</td><td><b>${money(r.sold_qty||0)}</b></td><td>${money(r.stock_qty||0)}</td><td><b>${fmt2(r.velocity||0)}</b></td><td>${fmt2(r.sell_through_pct||0)}%</td><td>${r.days_cover==null?'-':`${r.days_cover} ngày`}</td><td>${money(r.restock_target_days||14)} ngày</td><td><b>${money(r.restock_qty||0)}</b></td><td><span class="hint">${esc(r.restock_reason||'')}</span></td></tr>`).join('')}</tbody></table></div>`;
  }
...
async function renderProductAnalytics(){
  ...
  const t=data.totals||{};
  const kpis=`<div class="grid four pa-kpis"><div class="card kpi pa-kpi"><div class="label">Bán POS</div><div class="num">${money(t.pos_qty||0)}</div></div><div class="card kpi pa-kpi"><div class="label">Bán Online</div><div class="num">${money(t.online_qty||0)}</div></div><div class="card kpi pa-kpi"><div class="label">Tổng nhập / Tồn</div><div class="num">${money(t.import_qty||0)} / ${money(t.stock_qty||0)}</div></div><div class="card kpi pa-kpi"><div class="label">Có thể bán thêm / Miss sale</div><div class="num">${money(t.potential_sell_qty??t.sellable_qty??0)} / ${money(t.miss_sale_qty||0)}</div></div></div>`;
  const hiddenTabs=['speed','best'];
  if(hiddenTabs.includes(state.productAnalyticsTab))state.productAnalyticsTab='sales';
  const tabDefs=[['summary','Tổng hợp'],['sales','Chi tiết bán hàng'],['imports','Chi tiết nhập hàng'],['restock','Đề xuất tái'],['stock','Hàng tồn nhiều'],['slow','Hàng bán chậm']];if(isAdmin)tabDefs.push(['upload','Nhập file Sapo']);if(!isAdmin&&state.productAnalyticsTab==='upload')state.productAnalyticsTab='summary';
  ...
  let content='';
  if(state.productAnalyticsTab==='summary'){const best=(data.best_sellers||[]).slice(0,5),stock=(data.top_stock||[]).slice(0,5),slow=(data.slow_movers||[]).slice(0,5),restock=(data.restock_recommendations||[]).slice(0,5);const mini=(title,rows,field,suffix='')=>`<div class="card pa-mini"><h3>${title}</h3>${rows.length?rows.map((r,i)=>`<div class="pa-rank"><span class="badge dark">${i+1}</span><b>${esc(r.product_name||'')}</b><strong>${field==='velocity'?fmt2(r[field]||0):money(r[field]||0)}${suffix}</strong></div>`).join(''):'<div class="empty">Chưa có dữ liệu</div>'}</div>`;const restockMini=`<div class="card pa-mini pa-restock-mini"><div class="section-title"><h3>Đề xuất tái sớm</h3><span class="badge warn">${money((data.restock_recommendations||[]).length)} mã</span></div>${restock.length?restock.map((r,i)=>`<div class="pa-rank"><span class="badge ${r.restock_priority==='urgent'?'danger':r.restock_priority==='high'?'warn':'dark'}">${esc(r.restock_priority_label||'')}</span><b>${esc(r.product_name||'')}</b><strong>+${money(r.restock_qty||0)}</strong></div>`).join(''):'<div class="empty">Chưa có mã cần đề xuất tái</div>'}<div style="margin-top:10px"><button class="btn small secondary pa-open-restock" type="button">Xem toàn bộ đề xuất tái</button></div></div>`;content=`<div class="grid three pa-summary-grid">${mini('Top bán chạy',best,'sold_qty',' món')}${mini('Tồn nhiều',stock,'stock_qty','')}${mini('Bán chậm / tồn nhiều',slow,'stock_qty',' tồn')}</div><div style="margin-top:14px">${restockMini}</div><div class="card" style="margin-top:14px"><div class="section-title"><h3>Chi tiết tổng hợp</h3><div class="row">${productAnalyticsSortControl('speed')}<span class="badge">${data.period_days||1} ngày</span></div></div>${productAnalyticsTable(data.rows||[],'speed')}</div>`;}
  ...
  else if(state.productAnalyticsTab==='imports')content=`<div class="card"><div class="section-title"><div><h3>Chi tiết nhập hàng</h3><p class="hint" style="margin:4px 0 0">Có thể bán thêm = tốc độ bán hiện tại × số ngày còn lại của kỳ nếu luôn đủ hàng. Miss sale ƯT = phần nhu cầu dự kiến vượt tồn cuối kỳ.</p></div>${productAnalyticsSortControl('imports')}</div>${productAnalyticsTable(data.top_imports||[],'imports')}</div>`;
  else if(state.productAnalyticsTab==='restock')content=`<div class="card"><div class="section-title"><div><h3>Đề xuất tái</h3><p class="hint" style="margin:4px 0 0">Ưu tiên sản phẩm đang bán tốt nhưng tồn không đủ theo tốc độ bán. SL đề xuất tái nhằm phủ tối thiểu 14 ngày; hàng mới bán nhanh có thể nâng mốc 21 ngày, và kỳ hiện tại được cộng thêm 7 ngày đệm để hạn chế hết hàng.</p></div><div class="row">${productAnalyticsSortControl('restock')}<span class="badge warn">${money((data.restock_recommendations||[]).length)} mã</span></div></div>${productAnalyticsTable(data.restock_recommendations||[],'restock')}</div>`;
  ...
  const uploadStatusBlock=(isAdmin&&state.productAnalyticsTab==='upload')?`<details class="card pa-upload-status-card pa-history-card pa-history-details" style="margin-top:14px">
    <summary class="pa-history-summary"><span><b>Lịch sử dữ liệu Sapo</b><small>${uploadBatches.length} lần upload</small></span><span class="badge">Xem chi tiết</span></summary>
    <div class="pa-history-body">
      ...
      <div class="pa-history-toolbar"><div class="pillbar pa-upload-filter"><button class="active" data-upload-type="all" type="button">Tất cả</button><button data-upload-type="sales" type="button">Doanh thu</button><button data-upload-type="inventory_detail" type="button">XNT</button></div></div>
  ...
  $('.pa-open-restock')?.addEventListener('click',()=>{state.productAnalyticsTab='restock';renderProductAnalytics();});
...
async function renderProductTraining() {
  ...
  const exportBtns = can('can_export') ? '<button class="btn secondary" data-export="product_trainings">Tải Excel đào tạo SP</button><button class="btn secondary" data-export="product_training_attempts">Tải Excel kết quả kiểm tra</button>' : '';
...
async function renderCdpOjti() {
  ...
  if (isCdp) {
    ...
    const form = canManage ? `<div id="cdpCreate" class="card cdp-form-card">
      ...
      <form id="cdpOjtiForm" class="grid three">
        ...
        <div class="field"><label>Nhân sự được chấm</label><select name="trainee_id" required>${traineeOptions}</select></div>
  ...
  shell(`${typeTabs}<div style="height:12px"></div><div class="card"><div class="toolbar"><h3 style="margin-right:auto">Danh mục vị trí</h3><div class="field" style="min-width:220px;margin:0"><select id="cdpStoreFilter">${storeOptions}</select></div></div>${positionTabs}</div><div style="height:12px"></div>${mainContent}`, 'CDP / OJTI', '' );
...
async function renderSchedule() {
  ...
  weekDatas.forEach(d => (d.employees || []).forEach(emp => monthEmployeeMap.set(Number(emp.id), emp)));
  const monthEmployees = Array.from(monthEmployeeMap.values()).sort((a,b) => String(a.role||'').localeCompare(String(b.role||'')) || String(a.full_name||'').localeCompare(String(b.full_name||''),'vi'));
  ...
  const makeRows = (week) => {
    ...
    const cells = week.dates.map((day) => {
      ...
      const isEligible = eligibleDates.has(day) || Boolean(row?.shift_id);
      if (!isEligible) return `<td class="schedule-view-cell schedule-not-eligible"><span class="hint">—</span></td>`;
  ...
  shell(`${filter}${scheduleTable}${shiftForm}`, 'Lịch làm việc', '');
...
async function renderReports() {
  ...
  const rows = data.performance.filter(r => {
    if (!reportStoreId || allowedStores.length <= 1) return true;
    const periodIds = Array.isArray(r.period_store_ids) ? r.period_store_ids.map(Number) : [];
    return Number(r.store_id) === Number(reportStoreId) || periodIds.includes(Number(reportStoreId));
  });
  ...
  const perfTable = rows.length ? `<div class="table-wrap"><table><thead><tr><th>Top</th><th>Nhân viên</th><th>Cửa hàng trong tháng</th><th>Điểm tổng</th><th>Công việc / 35</th><th>UPT / 30</th><th>Đào tạo & GUESTS / 20</th><th>Kỷ luật / 15</th><th>Khối lượng</th></tr></thead><tbody>${rows.map((r, i) => {
    const periodStores = Array.isArray(r.period_store_names) && r.period_store_names.length ? r.period_store_names : [r.store_name || ''];
    const storeLabel = periodStores.filter(Boolean).join(' → ');
    const storeBreakdown = Array.isArray(r.store_breakdown) ? r.store_breakdown.filter(x => Number(x.shifts || 0) || Number(x.tasks || 0) || Number(x.revenue || 0)) : [];
    const shiftHint = storeBreakdown.length > 1 ? storeBreakdown.map(x => `${esc(x.store_name || '')} ${Number(x.shifts || 0)} ca`).join(' • ') : '';
    const uptMethodHint = r.upt_target_method === 'weighted_by_store_shifts' ? 'Target UPT bình quân theo ca tại từng cửa hàng' : 'Target UPT tháng';
    const uptStoreHint = r.upt_target_method === 'weighted_by_store_shifts' && Array.isArray(r.upt_weighted_stores)
      ? r.upt_weighted_stores.filter(x => Number(x.shifts || 0) > 0).map(x => `${esc(x.store_name || '')}: ${Number(x.shifts || 0)} ca × ${Number(x.effective_target_upt || x.target_upt || 0).toFixed(2)}`).join(' • ')
      : '';
    return `<tr><td><span class="badge dark">#${i + 1}</span></td><td><b>${esc(r.full_name)}</b><br><span class="hint">${r.role==='manager'?'CHT':'NVBH'}</span></td><td><b>${esc(storeLabel)}</b>${shiftHint?`<br><span class="hint">${shiftHint}</span>`:''}</td><td><b>${Number(r.final_score || 0).toFixed(2)}/100</b>${Number(r.final_score||0)>100?'<br><span class="badge ok">Vượt 100</span>':''}</td><td><b>${Number(r.task_points_35 || 0).toFixed(2)}/35</b><br><span class="hint">Đúng hạn ${r.tasks_on_time}/${r.tasks_total} • Trễ ${r.tasks_late} • Không HT ${r.tasks_not_completed}</span></td><td><b>${Number(r.upt_points_30 || 0).toFixed(2)}/30</b><br><span class="hint">UPT đạt ${Number(r.upt||0).toFixed(2)} / Target ${Number(r.target_upt||0).toFixed(2)} • ${Number(r.upt_achievement_percent||0).toFixed(1)}% (cap 130%)</span><br><span class="hint">${uptMethodHint}${uptStoreHint?` • ${uptStoreHint}`:''}</span></td><td><b>${Number(r.training_guests_points_20 || 0).toFixed(2)}/20</b><br><span class="hint">GUESTS ${Number(r.guests_score||0)}% • Training quá hạn ${Number(r.training_overdue||0)}</span></td><td><b>${Number(r.discipline_points_15 || 0).toFixed(2)}/15</b><br><span class="hint">${Number(r.violations_count||0)} lỗi • trừ ${Number(r.violation_deductions||0)} điểm kỷ luật</span></td><td><b>${r.tasks_total}/${Number(r.task_standard_total || 0).toFixed(0)} việc</b><br><span class="hint">Chuẩn ${r.worked_shifts || 0} ca × 3 việc/ca${shiftHint?` • ${shiftHint}`:''}</span></td></tr><tr class="performance-breakdown-row"><td></td><td colspan="8"><div class="performance-task-breakdown">${(r.task_breakdown||[]).map(x=>`<span><b>${esc(x.label)}</b> ${Number(x.points||0).toFixed(2)}/${x.max_points}đ • ${x.assigned}/${Number(x.standard_count||0).toFixed(0)} việc</span>`).join('')}</div></td></tr>`;
  }).join('')}</tbody></table></div>` : '<div class="empty">Chưa có dữ liệu tổng hợp</div>';
  ...
  shell(`<div class="card"><div class="toolbar"><h3 style="margin-right:auto">Hiệu suất nhân viên</h3>${reportStoreField}${periodFilters}${can('can_export') ? '<button class="btn secondary" data-export="performance">Tải file tổng hợp điểm</button>' : ''}</div><div class="performance-weight-note"><b>Tổng điểm:</b> Công việc 35đ • UPT 30đ (vượt tối đa 130% = 39đ) • Đào tạo & GUESTS 20đ • Kỷ luật 15đ.</div>${perfTable}</div><div class="card performance-calc-card" style="margin-top:16px"><details><summary><b>Xem cách tính điểm</b></summary><div class="performance-calc-grid"><div><b>1. Công việc — 35đ</b><br><span class="hint">Chuẩn 3 việc/ca. Đúng hạn = 100%; trễ/quá hạn = 70%; không hoàn thành = 0%. Việc vượt quota làm đúng không bù điểm; việc vượt quota trễ/không HT vẫn bị trừ.</span></div><div><b>2. UPT — 30đ</b><br><span class="hint">Điểm = 30 × % đạt UPT. % đạt được cap 130%, nên tối đa 39đ. UPT đạt = tổng món / tổng bill trong tháng.</span></div><div><b>3. Đào tạo & GUESTS — 20đ</b><br><span class="hint">Lấy điểm GUESTS trung bình trong tháng; mỗi training bắt buộc quá hạn trừ 10 điểm khỏi chỉ số trước khi quy đổi × 20%.</span></div><div><b>4. Kỷ luật — 15đ</b><br><span class="hint">Chỉ số kỷ luật = 100 − tổng điểm vi phạm; Điểm kỷ luật = chỉ số × 15%.</span></div></div><div class="hint" style="margin-top:10px"><b>Luân chuyển cửa hàng:</b> UPT đạt cộng toàn bộ giao dịch của nhân viên trong tháng; Target UPT bình quân theo số ca thực tế tại từng cửa hàng khi làm từ 2 cửa hàng trở lên.</div></details></div><div class="card" style="margin-top:16px"><h3>Hiệu suất cửa hàng</h3>${storeTable || '<div class="empty">Chưa có điểm OPS/VM trong kỳ</div>'}</div>`, 'Tổng hợp điểm', `Hiệu suất theo tháng ${month}/${year}`);
...
async function renderAdmin() {
  const transferUserId = Number(state.adminTransferUserId || 0);
  const transferMonth = state.adminTransferMonth || currentMonthLocal();
  const [data, storeRes, transferRes] = await Promise.all([
    api('/api/users?status=all'),
    api('/api/stores?status=all'),
    transferUserId ? api(`/api/user-transfers?user_id=${transferUserId}&month=${encodeURIComponent(transferMonth)}`).catch(() => ({ transfers: [], month_summary: [] })) : Promise.resolve({ transfers: [], month_summary: [] })
  ]);
  ...
  const transferUser = data.users.find(u => Number(u.id) === Number(transferUserId));
  const transferHistory = Array.isArray(transferRes?.transfers) ? transferRes.transfers : [];
  const transferMonthSummary = Array.isArray(transferRes?.month_summary) ? transferRes.month_summary : [];
  const transferCurrentStoreId = Number(transferRes?.current_store_id || transferUser?.store_id || 0);
  const transferCurrentStoreName = transferRes?.current_store_name || transferUser?.store_name || '';
  const transferStoreOptions = storeRows.filter(st => String(st.status || 'active') === 'active' && Number(st.id) !== transferCurrentStoreId).map(st => `<option value="${st.id}">${esc(st.name)}</option>`).join('');
  const transferSummary = transferMonthSummary.length ? `<div class="transfer-month-summary">${transferMonthSummary.map(r => `<div class="transfer-summary-item"><div><b>${esc(r.store_name || '')}</b><span>${Number(r.shifts || 0)} ca • ${Number(r.tasks || 0)} việc</span></div><strong>${money(r.revenue || 0)}đ</strong><small>UPT ${Number(r.upt || 0).toFixed(2)} • ${money(r.bill_count || 0)} bill / ${money(r.item_count || 0)} món</small></div>`).join('')}</div>` : '<div class="empty">Chưa có LLV / công việc / doanh thu của nhân viên trong tháng này.</div>';
  const transferHistoryTable = transferHistory.length ? `<div class="table-wrap"><table><thead><tr><th>Loại</th><th>Từ</th><th>Đến</th><th>Hiệu lực</th><th>Kết thúc</th><th>Ghi chú</th><th>Thao tác</th></tr></thead><tbody>${transferHistory.map(r => `<tr><td>${r.transfer_type==='temporary'?'<span class="badge warning">Hỗ trợ tạm thời</span>':'<span class="badge ok">Chính thức</span>'}</td><td>${esc(r.from_store_name || '')}</td><td><b>${esc(r.to_store_name || '')}</b></td><td>${dOnly(r.effective_date)}</td><td>${r.end_date?dOnly(r.end_date):'—'}</td><td>${esc(r.note || '')}<div class="hint">Tạo bởi ${esc(r.created_by_name || '')}</div></td><td><button class="btn small danger deleteTransferBtn" data-id="${r.id}">Xóa</button></td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">Chưa có lịch sử điều chuyển.</div>';
  const transferCard = transferUser ? `<div class="card transfer-admin-card" style="margin-top:16px"><div class="toolbar"><div style="margin-right:auto"><h3>Điều chuyển nhân sự — ${esc(transferUser.full_name)}</h3><div class="hint">Cửa hàng hiện tại: <b>${esc(transferCurrentStoreName || 'Chưa gán')}</b></div></div><button class="btn secondary" id="cancelTransferUserBtn">Đóng</button></div><div class="transfer-rule-box"><b>Cách hệ thống áp dụng</b><span>Điều chuyển chính thức: từ ngày hiệu lực, quyền cửa hàng của nhân viên chuyển sang cửa hàng mới. CHT cửa hàng cũ vẫn xem được dữ liệu lịch sử trước ngày chuyển nhưng không giao việc mới sau ngày chuyển. Hỗ trợ tạm thời: giữ cửa hàng chính và chỉ mở thêm quyền tại cửa hàng hỗ trợ trong khoảng ngày đã chọn. LLV là căn cứ thực tế theo từng ngày.</span></div><form id="transferUserForm" class="grid three transfer-form"><input type="hidden" name="user_id" value="${transferUser.id}"><div class="field"><label>Cửa hàng hiện tại</label><input class="input readonly" value="${esc(transferCurrentStoreName || '')}" readonly></div><div class="field"><label>Cửa hàng điều chuyển</label><select class="input" name="to_store_id" required><option value="">Chọn cửa hàng</option>${transferStoreOptions}</select></div><div class="field"><label>Loại điều chuyển</label><select class="input" name="transfer_type" id="transferTypeSelect"><option value="permanent">Điều chuyển chính thức</option><option value="temporary">Hỗ trợ tạm thời</option></select></div><div class="field"><label>Ngày hiệu lực</label><input class="input" type="date" name="effective_date" value="${isoDateLocal(new Date())}" required></div><div class="field" id="transferEndDateField"><label>Ngày kết thúc <span class="hint">(chỉ hỗ trợ tạm thời)</span></label><input class="input" type="date" name="end_date"></div><div class="field"><label>Ghi chú</label><input class="input" name="note" placeholder="VD: Điều chuyển theo kế hoạch tháng 9"></div><div style="grid-column:1/-1"><button class="btn">Lưu điều chuyển</button></div></form><div class="transfer-section-head"><div><h3>LLV & doanh thu theo cửa hàng</h3><p>Dữ liệu của ${esc(transferUser.full_name)} trong tháng đang xem.</p></div><div class="field"><label>Tháng xem</label><input class="input" type="month" id="adminTransferMonthInput" value="${esc(transferMonth)}"></div></div>${transferSummary}<div class="transfer-section-head"><div><h3>Lịch sử điều chuyển</h3><p>Không xóa dữ liệu LLV, task hay doanh thu cũ khi chuyển cửa hàng.</p></div>${can('can_export')?'<button class="btn secondary" data-export="user_transfers">Tải Excel điều chuyển</button>':''}</div>${transferHistoryTable}</div>` : '';
  const table = `<div class="table-wrap"><table><thead><tr><th>Họ tên</th><th>Tài khoản</th><th>Vai trò</th><th>Cửa hàng áp dụng</th><th>Trạng thái</th><th>Quyền</th><th>Thao tác</th></tr></thead><tbody>${data.users.map(u => { const hasView = Number(u.permissions.can_view_sales_target) === 1 && Number(u.permissions.can_view_store_sales_summary) === 1 && Number(u.permissions.can_view_bonuses) === 1; const hasTarget = Number(u.permissions.can_set_sales_targets) === 1; const inactive = u.status !== 'active'; const action = Number(u.id) === Number(state.user.id) ? '<span class="hint">Tài khoản hiện tại</span>' : `<div class="row wrap"><button class="btn small secondary editUserBtn" data-id="${u.id}">Sửa quyền</button>${u.role==='employee'?`<button class="btn small secondary transferUserBtn" data-id="${u.id}">Điều chuyển</button>`:''}<button class="btn small secondary quickPermBtn" data-id="${u.id}" data-mode="${hasView ? 'revoke' : 'grant'}">${hasView ? 'Thu hồi xem %/thưởng' : 'Cấp xem %/thưởng'}</button><button class="btn small secondary quickTargetBtn" data-id="${u.id}" data-mode="${hasTarget ? 'revoke' : 'grant'}">${hasTarget ? 'Thu hồi set target' : 'Cấp set target'}</button>${inactive ? `<button class="btn small ok restoreUserBtn" data-id="${u.id}" data-name="${esc(u.full_name)}">Mở lại</button>` : (can('can_delete_users') ? `<button class="btn small danger deleteUserBtn" data-id="${u.id}" data-name="${esc(u.full_name)}">Xóa vĩnh viễn</button>` : '')}</div>`; return `<tr class="${inactive ? 'user-inactive-row' : ''}"><td><b>${esc(u.full_name)}</b>${inactive ? '<div class="hint">Dữ liệu cũ vẫn được giữ trong báo cáo</div>' : ''}</td><td>${esc(u.username)}</td><td>${roleLabel(u.role)}</td><td>${esc((u.store_names && u.store_names.length ? u.store_names.join(' • ') : (u.store_name || '')))}</td><td>${userStatusBadge(u.status)}</td><td>${Object.entries(PERM_LABELS).filter(([k]) => Number(u.permissions[k]) === 1).map(([,l]) => `<span class="badge">${esc(l)}</span>`).join(' ')}</td><td>${action}</td></tr>`; }).join('')}</tbody></table></div>`;
  ...
  $$('.transferUserBtn').forEach(btn => btn.addEventListener('click', () => { state.adminTransferUserId = Number(btn.dataset.id); state.adminEditUserId = null; state.adminTransferMonth = currentMonthLocal(); renderAdmin(); window.scrollTo({ top: 0, behavior: 'smooth' }); }));
  $('#cancelTransferUserBtn')?.addEventListener('click', () => { state.adminTransferUserId = null; renderAdmin(); });
  $('#adminTransferMonthInput')?.addEventListener('change', e => { state.adminTransferMonth = e.target.value || currentMonthLocal(); renderAdmin(); });
  $('#transferTypeSelect')?.addEventListener('change', e => { const f = $('#transferEndDateField'); if (f) f.classList.toggle('transfer-end-active', e.target.value === 'temporary'); });
  $('#transferUserForm')?.addEventListener('submit', async e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const payload = Object.fromEntries(fd);
    if (payload.transfer_type !== 'temporary') payload.end_date = '';
    try {
      await api('/api/user-transfers', { method: 'POST', body: JSON.stringify(payload) });
      toast(payload.transfer_type === 'temporary' ? 'Đã lưu lịch hỗ trợ tạm thời' : 'Đã lưu điều chuyển nhân sự');
      await loadBase();
      renderAdmin();
    } catch (err) { toast(err.message, 'danger'); }
  });
  $$('.deleteTransferBtn').forEach(btn => btn.addEventListener('click', async () => {
    if (!confirm('Xóa bản ghi điều chuyển này? Dữ liệu LLV, task và doanh thu đã phát sinh sẽ không bị xóa.')) return;
    try { await api(`/api/user-transfers/${btn.dataset.id}`, { method: 'DELETE' }); toast('Đã xóa bản ghi điều chuyển'); await loadBase(); renderAdmin(); } catch (err) { toast(err.message, 'danger'); }
  }));
  ...
  $$('.deleteUserBtn').forEach(btn => btn.addEventListener('click', async () => {
    ...
    if (!confirm(`Xóa vĩnh viễn ${name}? Tài khoản sẽ bị khóa đăng nhập; dữ liệu lịch sử vẫn được giữ để báo cáo không sai.`)) return;
    try {
