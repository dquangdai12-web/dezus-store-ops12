...
function userTransferRows(userId) {
  return (db.user_transfers || [])
    .filter(r => r.status !== 'deleted' && Number(r.user_id) === Number(userId))
    .slice()
    .sort((a, b) => String(a.effective_date || '').localeCompare(String(b.effective_date || '')) || Number(a.id || 0) - Number(b.id || 0));
}
function effectivePermanentStoreId(row, atDate = null) {
  if (!row) return null;
  const rawIds = rawUserStoreIds(row);
  const fallback = Number(row.store_id || rawIds[0] || 0) || null;
  if (!row.transfer_managed || row.role !== 'employee') return fallback;
  const d = dateOnly(atDate || new Date());
  const permanent = userTransferRows(row.id).filter(r => String(r.transfer_type || 'permanent') === 'permanent');
  if (!permanent.length) return fallback;
  let storeId = Number(permanent[0].from_store_id || fallback || 0) || null;
  permanent.forEach(r => {
    if (String(r.effective_date || '') <= d) storeId = Number(r.to_store_id || storeId || 0) || storeId;
  });
  return storeId || fallback;
}
function effectiveStoreIdsForUserOnDate(row, atDate = null) {
  if (!row) return [];
  if (row.role !== 'employee' || !row.transfer_managed) return rawUserStoreIds(row);
  const d = dateOnly(atDate || new Date());
  const ids = [];
  const permanentStore = effectivePermanentStoreId(row, d);
  if (permanentStore) ids.push(permanentStore);
  userTransferRows(row.id).filter(r => String(r.transfer_type || '') === 'temporary').forEach(r => {
    const start = String(r.effective_date || '');
    const end = String(r.end_date || '');
    if (start && start <= d && (!end || d <= end)) ids.push(Number(r.to_store_id || 0));
  });
  return normalizeStoreIds(ids.length ? ids : rawUserStoreIds(row));
}
function getUserStoreIds(row) {
  return effectiveStoreIdsForUserOnDate(row, new Date());
}
function getPrimaryStoreId(row) {
  if (!row) return null;
  if (row.role === 'employee' && row.transfer_managed) return effectivePermanentStoreId(row, new Date());
  const raw = rawUserStoreIds(row);
  return row.store_id ? Number(row.store_id) : (raw[0] || null);
}
function userHasStore(user, storeId, atDate = null) {
  if (isAllStoreRole(user)) return true;
  if (!storeId) return true;
  const ids = atDate ? effectiveStoreIdsForUserOnDate(user, atDate) : getUserStoreIds(user);
  if (!ids.length) return false;
  return ids.includes(Number(storeId));
}
function userAssignedToStoreOnDate(user, storeId, atDate) {
  if (!user || !storeId) return false;
  if (isAllStoreRole(user)) return true;
  return effectiveStoreIdsForUserOnDate(user, atDate).includes(Number(storeId));
}
function userAssignedToStoreDuringPeriod(user, storeId, start, end) {
  if (!user || !storeId) return false;
  if (!start || !end) return userHasStore(user, storeId);
  let d = dateOnly(start);
  const stop = dateOnly(end);
  let guard = 0;
  while (d < stop && guard < 370) {
    if (userAssignedToStoreOnDate(user, storeId, d)) return true;
    d = addDaysUtc(d, 1);
    guard += 1;
  }
  return false;
...
function publicUser(row) {
  ...
  return {
    ...
    transfer_managed: Boolean(row.transfer_managed),
    status: u.status,
...
function upsertOjtiLinkedTask(row, actor) {
  ...
  const trainees = normalizeIdArray(row.trainee_ids).filter(uid => {
    ...
    return u && u.role !== 'admin' && userCanWorkAtStoreOnDate(u, storeId, row.plan_date || new Date());
  });
...
function removeStoredEvidenceFiles(storedValue) {
  if (!storedValue) return;
  let items = [];
  if (Array.isArray(storedValue)) items = storedValue;
  else {
    try {
      const parsed = JSON.parse(String(storedValue));
      items = Array.isArray(parsed) ? parsed : [parsed];
    } catch (_err) {
      items = [storedValue];
    }
  }
  items.filter(Boolean).forEach(item => removeLocalEvidenceFile(item));
}
function clearDeletedTaskReferences(taskIds = [], assignmentIds = []) {
  ...
  db.cdp_ojti.forEach(row => {
    ...
    if (taskSet.has(Number(row.linked_task_id || 0))) row.linked_task_id = null;
    if (row.linked_task_ids && typeof row.linked_task_ids === 'object' && !Array.isArray(row.linked_task_ids)) {
      Object.keys(row.linked_task_ids).forEach(key => {
        if (taskSet.has(Number(row.linked_task_ids[key] || 0))) delete row.linked_task_ids[key];
      });
      if (!row.linked_task_id) row.linked_task_id = Object.values(row.linked_task_ids).map(Number).find(Boolean) || null;
    }
    if (Array.isArray(row.item_values)) {
      row.item_values.forEach(item => {
        if (assignmentSet.has(Number(item?.task_assignment_id || 0))) item.task_assignment_id = null;
      });
    }
  });
}
...
function userCanWorkAtStoreOnDate(user, storeId, workDate) {
  if (!user || !storeId) return false;
  const d = dateOnly(workDate || new Date());
  return userAssignedToStoreOnDate(user, storeId, d) || userWorkedAtStoreOnDate(user.id, storeId, d);
}
...
function syncFutureShiftTasksForSchedule(storeId, changedDates = [], actorId = null) {
  ...
  db.tasks.forEach(t => {
    ...
    const desiredUsers = Array.from(desired).filter(uid => {
      const u = getActiveUser(uid);
      return u && u.role !== 'admin' && userCanWorkAtStoreOnDate(u, storeId, workDate);
    });
...
function leaderboardRows(user, period, refDate, storeId = null, options = {}) {
  ...
  if (requestedStoreId) {
    people = salesStaffForStore(requestedStoreId, { status: statusMode, start, end });
...
function monthlyWorkStoreBreakdownForUser(userId, monthKeyValue) {
  ...
  const taskRows = (db.task_assignees || []).filter(ta => Number(ta.user_id) === Number(userId)).map(ta => {
    const task = (db.tasks || []).find(t => Number(t.id) === Number(ta.task_id));
    if (!task) return null;
    const workDate = dateOnly(task.task_date || task.due_at || task.created_at || '');
    if (workDate < start || workDate >= end) return null;
    return task;
  }).filter(Boolean);
  const salesRows = (db.sales || []).filter(r => Number(r.user_id) === Number(userId) && monthKey(r.sale_date) === monthKeyValue);

  const storeIds = new Set([
    ...scheduleRows.map(r => Number(r.store_id || 0)),
    ...taskRows.map(r => Number(r.store_id || 0)),
    ...salesRows.map(r => Number(r.store_id || 0)),
  ].filter(Boolean));

  const rows = Array.from(storeIds).map(storeId => {
    const shifts = new Set(scheduleRows.filter(r => Number(r.store_id) === storeId).map(r => String(r.work_date || '')).filter(Boolean)).size;
    const tasks = taskRows.filter(r => Number(r.store_id) === storeId).length;
    const storeSales = salesRows.filter(r => Number(r.store_id) === storeId);
    ...
    return {
      ...
      upt: billCount ? Math.round((itemCount / billCount) * 100) / 100 : 0,
      target_upt: storeTargetUpt,
    };
  }).sort((a, b) => b.shifts - a.shifts || b.revenue - a.revenue || a.store_name.localeCompare(b.store_name, 'vi'));

  return { start, end, scheduleRows, taskRows, salesRows, rows };
}

function weightedMonthlyUptTargetForMovedUser(user, monthKeyValue, breakdown) {
  const ownTarget = Number(monthlyKpiTargetsForUser(user.id, monthKeyValue).target_upt || 0);
  const shiftRows = (breakdown?.rows || []).filter(r => Number(r.shifts || 0) > 0);
  // Chỉ dùng bình quân theo ca khi thực tế có ca ở từ 2 cửa hàng trở lên.
  if (shiftRows.length < 2) return { target_upt: ownTarget, method: 'monthly_employee_target', weighted_stores: [] };
  const totalShifts = shiftRows.reduce((sum, r) => sum + Number(r.shifts || 0), 0);
  if (!totalShifts) return { target_upt: ownTarget, method: 'monthly_employee_target', weighted_stores: [] };
  const weightedStores = shiftRows.map(r => ({
    ...r,
    effective_target_upt: Number(r.target_upt || ownTarget || 0),
  }));
  const valid = weightedStores.filter(r => Number(r.effective_target_upt || 0) > 0);
  if (!valid.length) return { target_upt: ownTarget, method: 'monthly_employee_target', weighted_stores: weightedStores };
  const weighted = valid.reduce((sum, r) => sum + Number(r.effective_target_upt || 0) * Number(r.shifts || 0), 0) /
    valid.reduce((sum, r) => sum + Number(r.shifts || 0), 0);
  return {
    target_upt: Math.round(weighted * 100) / 100,
    method: 'weighted_by_store_shifts',
    weighted_stores: weightedStores,
  };
}
...
function computePerformance(scopeUser, year = null, month = null) {
  ...
  if (scopeUser.role === 'manager') {
    const managerStoreIds = getUserStoreIds(scopeUser).map(Number);
  ...
  return users.map(u => {
    ...
    const movement = monthlyWorkStoreBreakdownForUser(u.id, performanceMonthKey);
    const monthSchedules = movement.scheduleRows;
    ...
    // UPT hiệu suất khóa theo đúng tháng đang xem.
    // Nhân viên luân chuyển 2+ cửa hàng trong tháng:
    // - UPT đạt = tổng món / tổng bill của chính nhân viên trên TẤT CẢ cửa hàng phát sinh trong tháng.
    // - Target UPT = bình quân Target UPT của từng cửa hàng theo số ca thực tế tại cửa hàng đó.
    // Nếu chỉ làm 1 cửa hàng (hoặc thiếu dữ liệu phân ca), giữ Target UPT tháng của nhân viên như trước.
    let upt = 0;
    let targetUpt = 0;
    let uptTargetMethod = 'monthly_employee_target';
    let uptWeightedStores = [];
    if (u.role === 'manager') {
      const managerShiftStores = (movement.rows || []).filter(r => Number(r.shifts || 0) > 0);
      const managerStoreIds = managerShiftStores.length ? managerShiftStores.map(r => Number(r.store_id)) : [Number(getPrimaryStoreId(u))].filter(Boolean);
      const storeRows = (db.sales || []).filter(sa => managerStoreIds.includes(Number(sa.store_id)) && monthKey(sa.sale_date) === performanceMonthKey);
      ...
      if (managerShiftStores.length >= 2) {
        const totalShifts = managerShiftStores.reduce((sum, r) => sum + Number(r.shifts || 0), 0);
        const valid = managerShiftStores.map(r => ({ ...r, effective_target_upt: Number(r.target_upt || 0) })).filter(r => r.effective_target_upt > 0);
        if (valid.length && totalShifts) {
          const validShiftTotal = valid.reduce((sum, r) => sum + Number(r.shifts || 0), 0);
          targetUpt = validShiftTotal ? Math.round((valid.reduce((sum, r) => sum + r.effective_target_upt * Number(r.shifts || 0), 0) / validShiftTotal) * 100) / 100 : 0;
          uptTargetMethod = 'weighted_by_store_shifts';
          uptWeightedStores = managerShiftStores;
        }
      }
      if (!targetUpt) targetUpt = Number(aggregateKpiTargetsForStore(getPrimaryStoreId(u), [performanceMonthKey]).target_upt || 0);
    } else {
      const employeeRows = movement.salesRows;
      ...
      const weightedTarget = weightedMonthlyUptTargetForMovedUser(u, performanceMonthKey, movement);
      targetUpt = Number(weightedTarget.target_upt || 0);
      uptTargetMethod = weightedTarget.method;
      uptWeightedStores = weightedTarget.weighted_stores || [];
    }
    ...
    return {
      user_id: u.id, role: u.role, full_name: u.full_name, store_id: getPrimaryStoreId(u),
      store_name: getStore(getPrimaryStoreId(u))?.name || '',
      period_store_ids: (movement.rows || []).map(r => Number(r.store_id)),
      period_store_names: (movement.rows || []).map(r => r.store_name).filter(Boolean),
      store_breakdown: movement.rows || [],
      ...
      upt_target_method: uptTargetMethod, upt_weighted_stores: uptWeightedStores,
...
app.get('/api/bootstrap', requireAuth, (req, res) => {
  ...
  if (req.user.role === 'manager') users = users.filter(u => getUserStoreIds(u).some(storeId => userHasStore(req.user, storeId)));
...
function transferView(row) {
  ...
  const creator = getUser(row.created_by);
  return {
    ...clone(row),
    user_name: user?.full_name || '',
    from_store_name: getStore(row.from_store_id)?.name || '',
    to_store_name: getStore(row.to_store_id)?.name || '',
    created_by_name: creator?.full_name || '',
  };
}

app.get('/api/user-transfers', requireAuth, requirePerm('can_manage_users'), (req, res) => {
  const userId = Number(req.query.user_id || 0);
  let rows = (db.user_transfers || []).filter(r => r.status !== 'deleted');
  if (userId) rows = rows.filter(r => Number(r.user_id) === userId);
  rows = rows.slice().sort((a, b) => String(b.effective_date || '').localeCompare(String(a.effective_date || '')) || Number(b.id || 0) - Number(a.id || 0));
  const user = userId ? getUser(userId) : null;
  const month = /^\d{4}-\d{2}$/.test(String(req.query.month || '')) ? String(req.query.month) : dateOnly(new Date()).slice(0, 7);
  const movement = user ? monthlyWorkStoreBreakdownForUser(user.id, month) : { rows: [] };
  res.json({
    transfers: rows.map(transferView),
    user: user ? publicUser(user) : null,
    current_store_id: user ? getPrimaryStoreId(user) : null,
    current_store_name: user ? (getStore(getPrimaryStoreId(user))?.name || '') : '',
    month,
    month_summary: movement.rows || [],
  });
});

app.post('/api/user-transfers', requireAuth, requirePerm('can_manage_users'), (req, res) => {
  const { user_id, to_store_id, effective_date, end_date, transfer_type = 'permanent', note = '' } = req.body || {};
  const user = getActiveUser(Number(user_id));
  if (!user || user.role !== 'employee') return res.status(400).json({ error: 'Chỉ điều chuyển tài khoản nhân viên đang hoạt động' });
  const toStore = getStore(Number(to_store_id));
  if (!toStore || String(toStore.status || 'active') !== 'active') return res.status(400).json({ error: 'Cửa hàng điều chuyển không hợp lệ' });
  const type = ['permanent','temporary'].includes(String(transfer_type)) ? String(transfer_type) : 'permanent';
  const eff = dateOnly(effective_date || new Date());
  if (!/^\d{4}-\d{2}-\d{2}$/.test(eff)) return res.status(400).json({ error: 'Ngày hiệu lực không hợp lệ' });
  const prevDate = addDaysUtc(eff, -1);
  const fromStoreId = Number(effectivePermanentStoreId(user, prevDate) || getPrimaryStoreId(user) || 0);
  if (!fromStoreId) return res.status(400).json({ error: 'Nhân viên chưa có cửa hàng hiện tại' });
  if (Number(toStore.id) === fromStoreId) return res.status(400).json({ error: 'Cửa hàng điều chuyển phải khác cửa hàng hiện tại' });
  let end = end_date ? dateOnly(end_date) : '';
  if (type === 'temporary') {
    if (!end) return res.status(400).json({ error: 'Hỗ trợ tạm thời cần ngày kết thúc' });
    if (end < eff) return res.status(400).json({ error: 'Ngày kết thúc phải sau hoặc bằng ngày hiệu lực' });
  } else end = '';
  if (type === 'permanent' && (db.user_transfers || []).some(r => r.status !== 'deleted' && Number(r.user_id) === Number(user.id) && String(r.transfer_type || 'permanent') === 'permanent' && String(r.effective_date || '') === eff)) {
    return res.status(400).json({ error: 'Nhân viên đã có điều chuyển chính thức ở ngày này' });
  }
  db.user_transfers = db.user_transfers || [];
  const row = {
    id: nextId('user_transfers'), user_id: user.id, from_store_id: fromStoreId, to_store_id: Number(toStore.id),
    transfer_type: type, effective_date: eff, end_date: end || null, note: String(note || '').trim().slice(0, 1000),
    status: 'active', created_by: req.user.id, created_at: nowIso(), updated_by: req.user.id, updated_at: nowIso()
  };
...
app.delete('/api/user-transfers/:id', requireAuth, requirePerm('can_manage_users'), (req, res) => {
  const row = (db.user_transfers || []).find(r => Number(r.id) === Number(req.params.id) && r.status !== 'deleted');
  if (!row) return res.status(404).json({ error: 'Không tìm thấy lịch sử điều chuyển' });
  const user = getUser(row.user_id);
  row.status = 'deleted';
  row.deleted_by = req.user.id;
  row.deleted_at = nowIso();
  if (user) {
    const remaining = userTransferRows(user.id);
    if (!remaining.length) {
      user.transfer_managed = false;
      const fallback = Number(row.from_store_id || user.store_id || rawUserStoreIds(user)[0] || 0) || null;
      user.store_id = fallback;
      user.store_ids = fallback ? [fallback] : [];
    } else {
      user.transfer_managed = true;
      const current = effectivePermanentStoreId(user, new Date());
      if (current) { user.store_id = current; user.store_ids = [current]; }
    }
  }
  saveDb();
  res.json({ ok: true });
...
app.delete('/api/stores/:id', requireAuth, requirePerm('can_manage_users'), (req, res) => {
  ...
  if (hasRef(db.user_transfers, x => x.status !== 'deleted' && (Number(x.from_store_id) === id || Number(x.to_store_id) === id))) refs.push('lịch sử điều chuyển');
...
app.delete('/api/users/:id', requireAuth, requirePerm('can_delete_users'), (req, res) => {
  ...
  db.user_transfers = (db.user_transfers || []).filter(x => Number(x.user_id) !== id);
...
function resolveTaskBulkSelection(payload = {}) {
  db.tasks = db.tasks || [];
  db.task_assignees = db.task_assignees || [];
  const mode = String(payload.mode || 'filtered') === 'all' ? 'all' : 'filtered';
  if (mode === 'all') {
    return {
      mode,
      assignment_ids: db.task_assignees.map(x => Number(x.id)),
      affected_task_ids: db.tasks.map(x => Number(x.id)),
      removable_task_ids: db.tasks.map(x => Number(x.id)),
      filters: { mode: 'all' }
    };
  }

  const storeId = Number(payload.store_id || 0) || 0;
  const userId = Number(payload.user_id || 0) || 0;
  const rawCategory = String(payload.category || '').trim();
  const category = rawCategory && rawCategory !== 'all' ? normalizeTaskCategory(rawCategory) : '';
  const fromDate = /^\d{4}-\d{2}-\d{2}$/.test(String(payload.from_date || '')) ? String(payload.from_date) : '';
  const toDate = /^\d{4}-\d{2}-\d{2}$/.test(String(payload.to_date || '')) ? String(payload.to_date) : '';
  const allowedStatuses = new Set(['assigned','overdue','not_completed','completed_on_time','completed_late']);
  const statuses = new Set((Array.isArray(payload.statuses) ? payload.statuses : []).map(String).filter(x => allowedStatuses.has(x)));
  if (fromDate && toDate && fromDate > toDate) throw new Error('Khoảng ngày xóa không hợp lệ');
  if (!statuses.size) throw new Error('Vui lòng chọn ít nhất một trạng thái công việc cần xóa');

  const assignmentIds = [];
  const affectedTaskIds = new Set();
  db.task_assignees.forEach(ta => {
    const t = db.tasks.find(x => Number(x.id) === Number(ta.task_id));
    if (!t) return;
    const workDate = taskWorkDate(t);
    if (storeId && Number(t.store_id) !== storeId) return;
    if (userId && Number(ta.user_id) !== userId) return;
    if (category && normalizeTaskCategory(t.category) !== category) return;
    if (fromDate && workDate < fromDate) return;
    if (toDate && workDate > toDate) return;
    const status = taskStatus({ ...ta, due_at: t.due_at });
    if (!statuses.has(status)) return;
    assignmentIds.push(Number(ta.id));
    affectedTaskIds.add(Number(t.id));
  });

  const assignmentSet = new Set(assignmentIds);
  const removableTaskIds = Array.from(affectedTaskIds).filter(taskId => {
    const rows = db.task_assignees.filter(a => Number(a.task_id) === Number(taskId));
    return rows.length > 0 && rows.every(a => assignmentSet.has(Number(a.id)));
  });
  return {
    mode,
    assignment_ids: assignmentIds,
    affected_task_ids: Array.from(affectedTaskIds),
    removable_task_ids: removableTaskIds,
    filters: { store_id: storeId || null, user_id: userId || null, category: category || 'all', from_date: fromDate || null, to_date: toDate || null, statuses: Array.from(statuses) }
  };
}

app.post('/api/tasks/bulk-delete', requireAuth, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Chỉ Admin được quyền xóa dữ liệu phân việc hàng loạt' });
  let selection;
  try { selection = resolveTaskBulkSelection(req.body || {}); }
  catch (err) { return res.status(400).json({ error: err.message || 'Bộ lọc xóa không hợp lệ' }); }
  const dryRun = Number(req.body?.dry_run || 0) === 1 || req.body?.dry_run === true;
  const matchedAssignments = selection.assignment_ids.length;
  const affectedTasks = selection.affected_task_ids.length;
  const removableTasks = selection.removable_task_ids.length;
  ...
  if (selection.mode === 'all') selection.affected_task_ids.forEach(id => orphanTaskIds.add(Number(id)));
  else selection.affected_task_ids.forEach(taskId => {
    if (!(db.task_assignees || []).some(a => Number(a.task_id) === Number(taskId))) orphanTaskIds.add(Number(taskId));
  });
  const deletedTaskIds = Array.from(orphanTaskIds);
  db.tasks = (db.tasks || []).filter(t => !orphanTaskIds.has(Number(t.id)) && !forcedTaskSet.has(Number(t.id)));
  clearDeletedTaskReferences(deletedTaskIds, selection.assignment_ids);
  ...
  db.task_delete_logs.push({
    ...
    deleted_assignments: matchedAssignments,
...
app.patch('/api/tasks/:taskId', requireAuth, requirePerm('can_edit_tasks'), (req, res) => {
  ...
  if (requestedIds) {
    const validIds = requestedIds.filter(uid => {
      const u = getActiveUser(uid);
      return u && u.role !== 'admin' && userCanWorkAtStoreOnDate(u, task.store_id, task.task_date || task.due_at || new Date());
    });
...
app.post('/api/sales/daily', requireAuth, requireAnyPerm('can_manage_total_sales','can_manage_sales'), (req, res) => {
  ...
  entries.forEach(entry => {
    ...
    if (!employee || employee.role !== 'employee' || !userCanWorkAtStoreOnDate(employee, storeId, sale_date)) return;
    saved.push(upsertSalesRow(employee, sale_date, entry, req.user.id, storeId));
  });
...
app.get('/api/store-staff', requireAuth, (req, res) => {
  const storeId = Number(req.query.store_id || getPrimaryStoreId(req.user) || 0);
  const store = getStore(storeId);
  if (!store) return res.status(400).json({ error: 'Cửa hàng không hợp lệ' });
...
app.post('/api/daily-report', requireAuth, requireAnyPerm('can_manage_daily_report','can_manage_sales'), (req, res) => {
  ...
  cleanSales.forEach(entry => {
    const employee = getActiveUser(Number(entry.user_id));
    if (!employee || employee.role !== 'employee' || (!userHasStore(employee, storeId) && !userWorkedAtStoreOnDate(employee.id, storeId, d))) return;
    savedSales.push(upsertSalesRow(employee, d, entry, req.user.id, storeId));
  });
...
app.post('/api/schedules/bulk', requireAuth, requirePerm('can_manage_schedule'), (req, res) => {
  ...
  entries.forEach(item => {
    ...
    if (!user || !userCanWorkAtStoreOnDate(user, storeId, item.work_date) || user.role === 'admin') return;
...
app.get('/api/export/:type.xlsx', requireAuth, requirePerm('can_export'), (req, res) => {
  ...
  } else if (type === 'user_transfers') {
    rows = (db.user_transfers || []).filter(x => x.status !== 'deleted').map(x => ({ id:x.id, employee_name:getUser(x.user_id)?.full_name||'', transfer_type:x.transfer_type==='temporary'?'Hỗ trợ tạm thời':'Điều chuyển chính thức', from_store:getStore(x.from_store_id)?.name||'', to_store:getStore(x.to_store_id)?.name||'', effective_date:x.effective_date||'', end_date:x.end_date||'', note:x.note||'', created_by:getUser(x.created_by)?.full_name||'', created_at:x.created_at||'' })).sort((a,b)=>String(b.effective_date).localeCompare(String(a.effective_date)));
  } else if (type === 'orders') {
